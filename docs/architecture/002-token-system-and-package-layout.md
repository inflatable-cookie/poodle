...
`@inflatable-cookie/poodle-core/styles` — plain global CSS, one kebab-case file per component,
unique `poodle-*` classes, data-attribute states, recipe hooks. Both web
frameworks import from it (`import "@inflatable-cookie/poodle-core/styles/button.css"`), so React
