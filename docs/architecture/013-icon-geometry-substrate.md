...
the same contour count and the same ordered open/closed signature. The first
registry never splits, merges, duplicates, or invents contours.
...
- `packages/core/src/icons/geometry-vectors.json` is the shared golden corpus
  consumed by TypeScript and Rust tests.
...
commands, unsupported input rejection, topology rejection, reverse direction,
closed-loop offsets, and deterministic correspondence in both languages.
