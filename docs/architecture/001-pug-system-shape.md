...
Pug should be structured around one contract surface and multiple
...
Pug adopts the W3C Design Tokens Community Group (DTCG) format (version 2025.10)
...
Pug uses **Style Dictionary 4.0** for multi-platform token emission.
...
All Pug components must have an approved contract document before
...
Pug should explicitly separate three component layers.
...
This layer is in scope for Pug because Loophole is not the only likely consumer
of workstation-style desktop UI.

## Extension Boundary

App-specific component systems should build above Pug.

Examples that should stay outside Pug core:
...
Pug should instead provide the tokens, shells, controls, tabs, surfaces, and
interaction rules those systems depend on.

## Underlay Integration Rule

Underlay should not become a thin public mirror of Pug. The intended shape is:

- Pug emits semantic tokens and reusable internal components
- Pug theme definitions should translate into both CSS/browser and Rust/GPUI
  forms from the same source model
- Underlay maps those tokens into Underlay-owned CSS/runtime systems
- Underlay wraps or reuses Pug components where it helps, while preserving
...
- Pug components wrap Bits primitives with token-based styling
- Bits is an implementation detail, not a public dependency
- Pug owns the public component contract

**Requirements:**
- Pug docs define the public contract
- Pug tokens define the styling vocabulary
- Pug theme definitions remain canonical even when realized as CSS custom
  properties or browser runtime helpers
- Pug decides what parity means
...
- Theme access via `cx.theme()` which uses Pug-generated token structs
...
Pug should optimize for Svelte and GPUI now, but the contract and token model
...
Pug themes should be authored once at the semantic layer and translated into
...
The current planning surface for the potential Pug suite is:
