...
<!-- pass: GPUI geometry + label-font now use the flat per-size contract table
     (switch_track/thumb/travel/label_font_rem helpers); Jetstream rebuilt —
     per-size geometry, space.inline.{xs,sm,md} gap token, custom on/off hex
     overrides, dual-label muted/active color rules, readOnly default cursor,
     and the Jetstream specimen now covers custom colors / dual+tones / sizes /
     densities / read-only (+ tooltip-grade probe tests). Remaining deltas are
     accepted runtime limits: interactive toggle (preview-loop, both targets),
     focus ring (JsEl has no :focus-visible hook — runtime/preview paints it),
     readOnly revert (state-driven), and ARIA (no a11y channel). -->
# Parity: Switch
...
- [x] FIXED **md geometry now follows Svelte's flat per-size literals.** `switch.rs` resolves
  track/thumb/travel from the new `presentation::switch_track_w_rem`/`switch_track_h_rem`/
  `switch_thumb_rem`/`switch_travel_rem` table (md `2.25rem × 1.375rem` / `1.125rem` / `0.875rem`),
  replacing the icon-formula scaling. Padding is `0.125rem` at every size; on-offset = padding + travel.
- [x] FIXED **Label font-size uses the switch-specific table.** `switch.rs` now calls
  `presentation::switch_label_font_rem` (xs `0.75`, sm `0.75`, md `0.8125`, lg `0.875`, xl `0.875`)
  instead of the shared `size_font_rem` ladder, which differed at xs (`0.6875`) and xl (`0.9375`).
- accepted: **No readOnly revert semantics** — `is_read_only` sets `cursor_default` and gates the click
  handler off, matching the contract effect (toggle suppressed). The native change-revert path is
  state-driven; acceptable runtime limit.
...
- [x] FIXED **Per-size geometry.** `effective_size` now drives track/thumb/travel via the new
  `presentation::switch_*_rem` table (shared values with GPUI). xs `1.75×1`/0.75; sm `2×1.125`/0.875;
  md `2.25×1.375`/1.125/0.875; lg `2.75×1.625`/1.375/1.125; xl `3×1.75`/1.5/1.25. Probe-tested
  (`per_size_track_scales`, `md_track_geometry_matches_svelte`).
- [x] FIXED **Gap token.** `gap` now resolves `space.inline.{xs,sm,md}` per density (compact/default/
  comfortable), matching GPUI and contract §8, replacing the `control_space_x_rem` heuristic.
- accepted: **No focus ring.** Contract §8 track `:focus-visible` outline is painted by the runtime/
  preview focus layer — JsEl has no `:focus-visible` hook and `js_switch` takes no focus-state input
  (same posture as tree/nav_card/number_input). The `accent.focusRing` token is resolved + referenced.
- [x] FIXED **Custom colors.** `off_color`/`on_color` hex overrides (#rgb/#rrggbb/#rrggbbaa) now win
  over tone tokens via `hex_to_rgb255`+`rgb255_to_vec4`, matching Svelte `switchStyles`. Probe-tested
  (`custom_on_color_tints_the_track_and_thumb`). Tones were already resolved.
- [x] **DONE: dual-label mode.** `leftLabel`/`rightLabel` flank the track; inactive sides now rest at
  `text-muted` and the active side re-tints to the off/on color (contract §8 "Label color rules"),
  replacing the earlier `text-secondary` inactive tint. Probe-tested.
- [x] FIXED **readOnly handling.** `is_read_only` now sets `cursor_default` (full opacity,
  non-interactive); disabled keeps not-allowed intent via `.disabled(true)` + opacity; interactive sets
  `cursor_pointer`. Revert-on-toggle is preview-loop.
- accepted: **No interactive toggle.** `js_switch` is render-only; toggle + Space/Enter live in the
  preview event loop (not yet wired). Consistent with the render-only component model.
...
- Jetstream covers: States (Off/On, Disabled off/on, Read-only on), Custom colors (on/off hex),
  Dual labels + tones (danger/success, warning/success), Sizes (xs–xl), Densities (compact/default/
  comfortable) (`jetstream/.../switch.rs`). — full specimen parity with Svelte; only live toggle
  interactivity remains preview-loop.
