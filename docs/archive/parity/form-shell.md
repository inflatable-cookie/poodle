...
<!-- jetstream-specimen pass: engine recovered; Jetstream specimen now mirrors GPUI coverage. Rebuilt packages/jetstream/preview/src/specimens/form_shell.rs driving the real js_form_shell across all four contract lifecycle states (ready / blocked → danger Callout + dimmed actions / busy → Pending Callout + loading primary / disabled → whole-shell dim). Header + two sections hosting real js_field+js_text_input slots + real js_callout StatusSummary + FormActions footer of real js_buttons. No fakes. Both previews build clean (gpui + jetstream → 0 errors). specimen flipped gap→ok. -->
