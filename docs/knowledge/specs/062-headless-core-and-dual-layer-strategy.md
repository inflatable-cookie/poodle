...
`g11.007`). Promote settled outcomes into `docs/knowledge/architecture/` and
...
- g11.001 modernized the full Svelte surface to runes/callback-first/snippet
  ...
  task g11.001 (Git history) are the
...
- ~~Recipe override delivery mechanism~~ **Resolved (g11.005):** CSS
  ...
  contract (`docs/knowledge/architecture/007-appearance-recipe-contract.md`), with a
  generated inventory
...
- machine model, package shape, adapter boundary → `docs/knowledge/architecture/`
...
- recipe override surface and stability guarantees → contract update layered
  on spec `026`
- consumer compatibility rule → `docs/knowledge/contracts/working-rules.md` if it
