...
Created: 2026-08-21
Card: task g15.044 (Git history)
Parent: task g15.012 (Git history)
Log: the retired logs (Git history)
Reproduction: the retired logs (Git history) (complete recipe)
Receipt: the retired logs (Git history) (verbatim run output)
...
the retired logs (Git history), which contains the complete
...
`button-offscreen.png` (g15.044 log assets, Git history) — 480×160 RGBA,
...
`mktemp -d` workspace produced that same hash, matching the PNG committed with the g15.044 log (Git history).
