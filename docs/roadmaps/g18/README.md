...
| [`g18.009`](009-v040-release-certification-and-desktop-unblock.md) — v0.4.0 release certification and Desktop unblock | complete | PR #266 (merge `4c0bb57410584da944f781eb8064805521fba054`); independent exact-head review, green `rust`/`web` checks; wrapper `4a39055f3` tagged `v0.4.0`; branch/tag/publish runs green; core+Svelte `0.4.0` on npm with attested provenance; fresh registry `./editor` type/SSR/browser proof; Desktop capsule returned |
...
| [`g18.033`](033-gpui-form-dialog-render-convergence.md) — GPUI FormDialog render convergence | ready | immediate repair: make the real submitting specimen's first draw converge, restore 175/175 probe proof, then complete one wholly green bounded board |
...
operator-approved g18.028 repaired those tests and merged PR #260. The resumed
...
hard-codes the old preview-lock hash and package versions. Merged g18.030
derives them from the lock and repins the complete current evidence in
...
code: root repository metadata stayed at `0.1.0`, and the GPUI census
generator embedded `0.3.0` in 65 receipts. Merged g18.031 (PR #264,
