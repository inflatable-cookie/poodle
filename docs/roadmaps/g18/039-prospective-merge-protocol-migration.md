# g18.039 — Prospective-merge protocol migration
...
Poodle's Queue control manifest uses `paseo.queue.control.v4` and evaluates the
...
- **State:** ready; configuration maintenance independent of g18 product work.
- **Owned mutable paths:** `.paseo/queue.json` only.
- **Worker:** automatic mechanical/general pool with independent review.
- **Excluded:** product code, releases, CI, product sequencing, Queue state and
  thread/workspace disposition.
...
Return to the existing g18 planning horizon. This task authorizes no product
successor.
