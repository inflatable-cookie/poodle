...
| Gap | Current evidence | Owner |
| --- | --- | --- |
| GPUI pixel capture boundary | Historical `g15.044`–`g15.051` fork/offscreen results remain retained evidence, but are superseded for shipped GPUI 0.2.2 by `g15.059`–`g15.061`: crates.io GPUI supports an operator-approved real non-activating window capture, not true headless/offscreen pixels. The retained Button comparison remains windowed; default QA/CI makes no GPUI pixel claim. | `g15.059`–`g15.061` supersede the capture transport; comparison evidence remains `g15.047` |
