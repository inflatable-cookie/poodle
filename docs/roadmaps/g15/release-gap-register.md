...
| AgentTranscript scroll/follow | — | `AgentTranscriptSpec` declares auto-scroll, pin threshold, and jump label, but `poodle-render` emits an unbounded column and no jump control | GPUI has no tracked viewport, detached-state observation, or real jump-to-bottom behavior; exposed by PR #48 | `g15.037` planned; blocks `g15.024`, `g15.026`, and `g15.013` |
