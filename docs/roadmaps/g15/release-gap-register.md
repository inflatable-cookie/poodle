...
| Specimen axis domains | EmptyState declares a two-value native size domain that its renderer does not consume; Icon declares a three-value native size domain and an unused density field | EmptyState and Icon renderers cannot yet supply the full web-admitted axis evidence | fake panes removed by `g15.019`; honest gaps remain | authority decision and bounded follow-up required before `g15.026` / `g15.013` |
