...
| `ErrorBoundary` | A | A | A | keep | human verdict (`g15.031`): keep after Sv/Rc specimen repair — caught-error child threw on every render, so Reset never showed recovery; instance-owned crash tokens in a shared `WeakSet` now throw once per token/remount, preserving recovery within an instance and a fresh error state after navigation remount; Gp keeps static error/normal evidence (no live throw cycle) |
