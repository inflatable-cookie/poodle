...
| `UiPresentationProvider` | complete | complete | `UiPresentationProvider.test.tsx` | `test:web-pack-install` | `UiPresentationProviderSpec` (`packages/contracts/components/src/ui_presentation_provider.rs`) | `missing` — declared capability absence; ambient presentation cannot yet cross a built Node tree | `packages/gpui/preview/src/specimens/ui_presentation_provider.rs` (explicit host-equivalent values; not cascade evidence) |
