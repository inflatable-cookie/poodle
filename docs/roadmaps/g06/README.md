...
a retained-mode UI system). A future Jetstream adapter (Pug g08) requires that
...
- spec crates live under `packages/gpui/` with `pug-gpui-*` naming
...
- shared contract crates are renamed and restructured as `pug-contracts-*`
...
Audit the existing `pug-gpui-*` crates and identify what is genuinely
...
- `pug-gpui-tokens` → `pug-tokens` (token constants and typed resolution)
- `pug-gpui-primitives` → `pug-primitives` (spec structs and shared enums)
- `pug-gpui-composites` → `pug-composites` (composite spec structs)
- `pug-gpui-workstation` → `pug-workstation` (workstation spec structs)

Preserve the existing `pug-gpui-*` crate names as thin re-export wrappers
...
Publish a document in the Pug repo that describes Jetstream's UI system
...
This document is the contract between Pug g08 and Jetstream g04.016.
