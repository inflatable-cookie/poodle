...
`g07` builds the GPUI rendering adapter — the code that takes Pug's
...
- shared contract crates (`pug-primitives`, `pug-composites`, `pug-workstation`,
  `pug-tokens`) provide renderer-agnostic specs for all 124 components
...
- GPUI rendering adapter crate (`pug-gpui`) implements the adapter trait for
...
Create `pug-gpui` crate that implements the renderer adapter trait from g06.007.
...
crate replaces the old `pug-gpui-*` re-export pattern with an actual rendering
implementation.
...
multi-app consumption of the `pug-gpui` crate. Document integration patterns
