...
- `g15` is the release-first v0.2.0 runway. The release-baseline roster, all
  ...
  dependency-only `g15.014` is in flight and native Licence card `g15.007` is
  ready in parallel. Jetstream remains deferred.
...
- `g15` — active v0.2.0 release baseline; `g15.001`–`g15.006` complete,
  `g15.014` in flight, `g15.007` ready in parallel
