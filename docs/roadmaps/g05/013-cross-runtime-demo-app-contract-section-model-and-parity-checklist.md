...
- attached the demo contract to both runtimes without turning runtime mechanics
  ...
  - Svelte rebuild bound to `g05.014`
  - GPUI demo implementation bound to `g05.015`
- extended `packages/svelte/preview/scripts/lint-docs.ts` so the new demo
  ...
  inventory
- rolled the status surfaces forward so the repo now points at `g05.014`

## Next Task

Open `g05.014` and rebuild the Svelte demo app against the explicit demo
