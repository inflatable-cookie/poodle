...
Flint's Svelte implementation needs an explicit relationship to Bits and to any
other lower-level browser/UI dependencies.

## Problem

If the Svelte side treats Bits as the public contract, Flint loses ownership of
...
- [ ] define what Flint must still own regardless of Bits usage
...
- [ ] define how Bits-backed components remain contract-owned by Flint
- [ ] define which concerns Bits is allowed to own: accessibility, focus,
  keyboard, state, overlay plumbing
- [ ] define which concerns Flint must own: public props, variants, tokens, docs,
  parity rules
...
- [ ] define how `class` and `data-*` driven token styling should work in the
  wrapper layer
- [ ] define when Flint should expose compound components versus simplified
  facades
...
- [ ] public contract ownership remains with Flint
