...
### Two Gaps Found On 2026-08-10

Both surfaced while repointing consumers onto the published packages, and both
were invisible to the work that created them.

**One repository was missed.** The consumer posture table above does not list
the desktop application of the private consumer, and it was never migrated. It
kept importing an icon name directly from `poodle-core/icons` that the pinned
manifest does not carry, so its `svelte-check` had been failing on that one
line ever since. It needed exactly one icon. Now generated like everyone else.

A table of postures is not a checklist of repositories. Nothing compared it
against the actual consumer set, so an absent row looked identical to a
migrated one.

**`poodle-icons` never worked as documented.** The command this card prescribes
— `bun x poodle-icons --names icons.json --out src/icons.generated.ts` — exited
0 and wrote nothing. npm installs a bin as a symlink, and the module's
entrypoint guard compared `import.meta.url` against an unresolved
`process.argv[1]`, so it never matched.

It survived because nothing ran it that way: `effigy icons:build` calls
`scripts/build-default-icons.ts` directly, and all sixteen consumer sets were
generated by hand during this card. The documented public interface was the one
path never exercised.

Fixed with `realpathSync`. The lesson generalises past icons: a card that
publishes a command should run that command exactly as written, through the
installed artifact, rather than the equivalent internal call.
