...
- `@types/react` added to `@poodle/react` so the components package
...
2. `import "@poodle/styles/<kebab>.css";` — never write CSS. Class names,
...
3. Props interface mirrors the Svelte `Props` shape. Svelte→React idiom
   ...
   `bind:this` → `useRef`.
4. Behavior comes from `@poodle/headless` — the same machine transition the
...
`@poodle/react`'s `index.ts` re-exports each component + its props type.
