...
| `g13-b008-split-button-tone` | `g13/batch-cards/008-split-button-tone-parity.md` | `thread/g13-008-split-button-tone-parity` | `deepseek-v4-flash` (`xhigh`) | dispatched | Carries the orchestrator shadow ruling (contract right, split-button.css outlier — button.css:193 ships the same shadow); delivers primary danger shadow + success/warning; enumerates visual diffs but refreshes no baseline |
| `g13-b009-dialog-initial-focus` | `g13/batch-cards/009-dialog-initial-focus.md` | `thread/g13-009-dialog-initial-focus` | `deepseek-v4-flash` (`xhigh`) | dispatched | Side-quest, outside the IR lane: consumer-driven `initialFocus` for Dialog/FormDialog in Svelte + React with contract updates; branched from `b1a4a5e7` so the owner's focus-guard work is the base |
