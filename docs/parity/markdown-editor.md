...
- [x] FIXED preview scroll ownership — preview pane declares `LayoutOverflow::Scroll` with `min_height: 0` / `fills_height`; body shrinks under a constrained root (`min_height: 0`, overflow hidden) so long preview content does not invent a tracked-scroll architecture (g16.035).
...
- Pass 42: g16.035 bounded preview scroll — shared CSS column flex + `max-height: 100%` + `min-height: 0` shrink chain; GPUI/render preview declares `LayoutOverflow::Scroll` with the same shrink ownership. Source-text-versus-rendered-HTML remains Tier-3.
