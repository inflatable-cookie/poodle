...
- [x] FIXED preview scroll ownership — preview pane declares `LayoutOverflow::Scroll` with `min_height: 0` / `fill_height`; body shrinks under a constrained root (`min_height: 0`, overflow hidden); mounted GPUI regression `markdown_editor_bounded_preview_scrolls_under_host_height` proves host-bounded geometry and wheel scroll (g16.035).
...
- Pass 42: g16.035 bounded preview scroll — shared CSS column flex + `max-height: 100%` + `min-height: 0` shrink chain; GPUI/render preview declares `LayoutOverflow::Scroll` with the same shrink ownership; mounted headless GPUI regression `markdown_editor_bounded_preview_scrolls_under_host_height` proves host-bounded geometry and wheel scroll hit-test. Source-text-versus-rendered-HTML remains Tier-3.
