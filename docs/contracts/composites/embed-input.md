...
| Error | `--pug-color-text-danger` | error text color (fallback #ef4444) |
| SuccessText | `--pug-color-text-success` | success text color (fallback #22c55e) |

## 9. Svelte Notes

- Uses `createEventDispatcher` for `parse` and `change` events
- Composes `TextArea` and `Pill` from `@pug/svelte-primitives`
...
- Expected crate/module surface: `pug_gpui::composites::embed_input`
- Provider detection logic should be shared via `pug-primitives` or a shared utility crate
