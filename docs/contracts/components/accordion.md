...
Semantic open state, `aria-expanded`, focusability, and panel ownership update
immediately in every policy. Authored initial state paints its endpoint.

- `full`: the panel may animate clipped block-axis height for 180ms and the
...
Unmount cancels the panel and indicator clocks. Native may use the named
static/opacity approximation from architecture 012; that remains a visual gap.
