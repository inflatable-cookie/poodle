...
| Single-paragraph list-item unwrap and flush-left inside markers are web-implementation-only | the web targets (Svelte and React) flatten the item so the marker sits on the text line; the natives keep block-wrapped items and outside markers | pending review | port to the natives when one is next touched |
