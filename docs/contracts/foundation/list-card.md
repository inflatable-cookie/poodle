...
- In scope: interactive and disabled states, leading shape variants, badges slot,
  footer slot with counter helper, context menu composition, title truncation,
  meta display with tabular-nums
...
|------|----------|-------------|---------------|
| Root | yes | flex row container | padding, border, radius, background, gap |
...
- Leading: fixed 2rem square, circle or rounded-square
...
- `data-disabled`, `data-leading-shape` data attributes on root
