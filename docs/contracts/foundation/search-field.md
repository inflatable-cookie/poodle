...
TextInput. The treatment CSS custom properties use the `--pug-text-input-*`
...
| `cursor` | `pointer` |
| `border-radius` | `calc(var(--pug-treatment-interactive-subtle-radius) - 0.0625rem)` |

### Clear Button hover

| Property | Value |
|----------|-------|
| `background` | `var(--pug-treatment-interactive-subtle-fill-hover, color-mix(in srgb, var(--pug-color-background-surface) 84%, transparent))` |
