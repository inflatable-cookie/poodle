...
| Var | Description |
|-----|-------------|
| `--pug-slider-percent` | computed as `((value - min) / (max - min)) * 100%`; drives fill width/height |
...
- Uses a native `<input type="range">` overlaid on a custom track/fill for
  visual rendering
- The CSS custom property `--pug-slider-percent` is computed reactively:
