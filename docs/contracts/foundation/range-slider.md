...
| Var | Description |
|-----|-------------|
| `--pug-range-start` | computed as `((lower - min) / (max - min)) * 100%`; positions fill start |
| `--pug-range-end` | computed as `((upper - min) / (max - min)) * 100%`; positions fill end |
...
| Property | Value |
|----------|-------|
| `box-shadow` | `0 0 0 0.1875rem color-mix(in srgb, var(--pug-color-accent-focusRing) 32%, transparent), 0 0.125rem 0.5rem color-mix(in srgb, black 18%, transparent)` |
...
- CSS custom properties `--pug-range-start` and `--pug-range-end` are computed
