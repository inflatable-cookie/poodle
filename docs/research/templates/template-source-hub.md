# Source Hub: [System Name]

Status: [planned | active (researching) | complete]
Created: YYYY-MM-DD
Updated: YYYY-MM-DD

## Purpose

Brief description of what external system this hub documents and what Poodle decisions it informs.

---

## Source Inventory

### Official Sources

| Source | URL | Type | Last Checked |
|--------|-----|------|--------------|
| | | | |

### Community/Third-Party

| Source | URL | Type | Notes |
|--------|-----|------|-------|
| | | | |

---

## Key Questions

### [Category 1]

- [ ] Question 1?
- [ ] Question 2?

### [Category 2]

- [ ] Question 3?
- [ ] Question 4?

---

## Findings

### [Category 1]

*(To be filled as research progresses)*

### [Category 2]

*(To be filled as research progresses)*

---

## Implications for Poodle

*(To be synthesized into translation memo)*

---

## Related

- Value track: [../value-tracks/...]()
- Milestone: [../../roadmaps/...]()

---

## Next Task

Clear next step for this research.
