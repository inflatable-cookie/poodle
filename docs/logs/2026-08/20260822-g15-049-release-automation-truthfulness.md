...
- `.github/workflows/ci-rust.yml` runs `effigy ci:rust`.
- `.github/workflows/ci-native.yml` runs `effigy ci:native`.
...
- `.github/workflows/release.yml` runs `effigy release gates` as its single
  release gate, then retains pack inspection and dry-run publication.
...
- `tasks/effigy.tasks.toml` removes `ci:conformance`, puts the static release
  automation check first in `qa`, and adds `check:release-automation`.
- `scripts/check-release-automation.ts` statically protects workflow pins,
  selector mappings, tier rejection, release posture, publish scope, and
  non-vacuous gate configuration.
...
`contents: read`, no `NPM_TOKEN`, Bun `1.3.14`, Node `22.22.2`, and npm
`12.0.2`. The selected npm CLI is above the documented npm `>=11.5.1` and
Node `>=22.14.0` trusted-publishing requirements.

## Validation

| Check | Result |
| --- | --- |
| `bun scripts/check-release-automation.ts` | pass — checked 5 retained workflows, the Effigy gate, alias removal, and publish set |
...
| `effigy ci:native` | pass — 56 headless regressions, 8 specimen probes, native and adapter suites |
...
| `git diff --check origin/main...HEAD` | pending final diff check |

The release-gate run proves the gate is configured and executed; its failure
is an existing dependency-license policy issue outside this batch. No release
workflow was dispatched, no windowed/native-visual/Jetstream selector was
run, and no tag, package, registry, or release mutation occurred.
