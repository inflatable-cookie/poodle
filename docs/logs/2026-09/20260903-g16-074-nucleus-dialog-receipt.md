...
The new regression `dialog_dismissal_axes_and_controlled_rebuild_reach_the_mounted_backend`
...
body composed through production `Surface` with panel tone and subtle border, actions composed
through production `Button`s with secondary, primary, and disabled states, mounted layout bounds
(positive dimensions, panel containment within backdrop, child containment within panel, vertical order),
...
`8d3cc4ff85f2a534a5ea6adbb76891023d17ae7b`. The ledger records 11 mounted Nucleus rows out of 29.
...
- Renderer:
  - `packages/render/src/dialog.rs`: assigned `poodle-dialog-surface` id to the dialog panel container, registered `poodle-dialog-layer` dismiss layer on both root backdrop and surface panel, wired `on_dismiss` reason handler for Escape and outside dismissal, assigned inert `on_activate` on panel to consume inside clicks, and added unit tests for backdrop/surface rendering, dismissal wiring, bare mode, and width presets.
- GPUI Node Backend:
...
- Headless Regressions:
  - `packages/gpui/preview/tests/headless_regressions.rs`: added `dialog_dismissal_axes_and_controlled_rebuild_reach_the_mounted_backend` across 6 phases:
    1. Production Spec & Token Structure Proof: root backdrop tokens, overlay posture, panel elevated styling, header title/description, close affordance, body Surface, and action Buttons.
...
- Receipts:
  ...
  - Refreshed existing receipts for `AppHeader`, `Button`, `Icon`, `IconButton`, `Menu`, `SegmentedControl`, `SplitView`, `Surface`, `Tabs`, `Text` with source commit `8d3cc4ff85f2a534a5ea6adbb76891023d17ae7b`.
- Manifest & Ledger:
  - `docs/roadmaps/g16/nucleus-parity-manifest.json`: updated Dialog `expected_test` to `dialog_dismissal_axes_and_controlled_rebuild_reach_the_mounted_backend` and `source_commit` to `8d3cc4ff85f2a534a5ea6adbb76891023d17ae7b`.
...
| Dependencies are real | replace Surface or Button with raw container | renderer-owned metadata or mounted activation fails |
| Input is mounted | invoke handlers directly | mounted observation or request/action trace is absent |
| Dismissal axes stay separate | couple Escape policy to backdrop policy | `assertion left == right failed: Escape key when dismiss_on_escape=false must be inert (left: 3, right: 2)` |
