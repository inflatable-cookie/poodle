...
receipts (Text, Surface) pin the exact runtime source commit `b0cdb23434036ed07c1a372603d675a3773e775f`.
The ledger records 5 mounted Nucleus rows.

## What landed

- Render fixes and unit tests:
  - `packages/render/src/surface.rs`: mapped `SurfaceRole` (Group, Region) and `spec.label` to `Node.a11y.role` and `Node.a11y.label`; resolved border width via `resolve_border_width`; added unit tests with exact assertions covering tones (Panel 96% alpha, Canvas 98% alpha, Elevated 96% mix over panel), borders (Subtle 74% alpha, Default 100%, width 1.0), elevation shadow (`ELEVATION_SURFACE`), radius (`RADIUS_SURFACE` on all 4 corners), panel insets across all 4 padding scales, and a dedicated test proving `surface` queries `resolve_border_width` rather than `resolve_space`.
...
- Receipts:
  ...
  - Refreshed existing receipts for `Button`, `Icon`, `IconButton` with source commit `b0cdb23434036ed07c1a372603d675a3773e775f`.
- Manifest & Ledger:
  - `docs/roadmaps/g16/nucleus-parity-manifest.json`: updated Text and Surface `expected_test` and `source_commit` (`b0cdb23434036ed07c1a372603d675a3773e775f`).
