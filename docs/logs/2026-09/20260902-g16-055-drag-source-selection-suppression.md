...
prior inline value and priority on every exit. An activated pointerup or cancel
arms a one-shot capture listener that stops the browser compatibility click on
that source path and prevents the cancelable click's default; the guard expires
on consume, the next press, timeout plus rAF, disconnect, or destroy. Taps and
pre-threshold releases still click. Keyboard pickup is unchanged. Svelte and
React inherit the controller. No Tree CSS, public API, or Rust/GPUI change.

## Evidence

- Controller: suppression at pointerdown before threshold; empty/`text`/`all`
  restore; `!important` value and priority restore for `user-select` and
  `-webkit-user-select`; pointercancel, Escape, source loss, disconnect,
  destroy; interactive, no-drag, contenteditable, disabled, and secondary-button
  exclusions; touch candidate plus tolerance cancel; overlapping roots;
  activated compatibility click consumed and default-prevented for
  committed/rejected/failed/cancelled; registered `<a href>` source does not
  follow the hash; async dropping click consumed; stale guard expires; tap and
  sub-threshold click still reach the source without preventDefault.
...
- Chromium and WebKit: reorderable label drag has empty Selection before and
  ...
  non-reorderable drag produces a non-empty range; registered link source drag
  does not set `#compat-click`; a tap on that link still does.
