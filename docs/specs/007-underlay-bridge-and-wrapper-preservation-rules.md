...
Freeze the baseline rules for Underlay adoption so Pug can be consumed
internally by Underlay without turning Pug into the public API for Underlay
applications.
...
without requiring app code to import Pug directly.
...
- token meaning canonical in Pug
...
Bridge token maps may rename Pug-emitted variables into Underlay-owned naming
where needed.

They must:

- point back to canonical Pug semantic paths
- remain traceable
...
The bridge may translate Pug theme identities into Underlay-owned runtime theme
registration shapes.
...
When Underlay wraps Pug components internally:

- the public Underlay component name remains Underlay-owned
- the public Underlay prop contract remains Underlay-owned
- migration from old Underlay implementation to Pug-backed internals must be
...
Wrappers may not expose raw Pug component contracts as the new default API for
Underlay apps.
...
- or Pug-specific layout-layer terminology
...
Bridge wrappers must preserve Pug accessibility semantics when adapting APIs.
...
- state ownership differences between Underlay components and Pug contracts
...
- `packages/bridges/underlay/css/pug-to-underlay.css`
