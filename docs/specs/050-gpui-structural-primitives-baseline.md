...
`poodle-gpui-primitives` is now a preview-channel public-intent Rust crate.
...
focus-treatment semantics from `poodle-gpui-tokens`, not from local Rust-only
constants.
...
`poodle-gpui-primitives` now sits between token bindings and later GPUI runtime
depth:
