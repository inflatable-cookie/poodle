...
and text-entry primitive semantics in `poodle-gpui-primitives` so later GPUI
composites build on the same button, field, and text-entry meaning as Svelte.

## Package Rule

The `g04.004` tranche extends `poodle-gpui-primitives` with:
...
Action, field, and text-entry primitives must resolve from `poodle-gpui-tokens`
for at least:
