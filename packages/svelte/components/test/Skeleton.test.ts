...
describe("Skeleton (svelte)", () => {
  ...
  it("resolves circle shape to square rem dimensions and hides it from assistive tech", () => {
    const root = rootOf(render(Skeleton, { props: { shape: "circle" } }).container);
  ...
  it("applies explicit width and height overrides", () => {
    const root = rootOf(
      render(Skeleton, { props: { width: "12rem", height: "3rem" } }).container,
    );
  ...
  it("renders the table-row preset anatomy when preset is set", () => {
    const { container } = render(Skeleton, { props: { preset: "table-row", animated: false } });
  ...
  it("honours the detail-section preset line count", () => {
    const { container } = render(Skeleton, { props: { preset: "detail-section", lines: 5 } });
