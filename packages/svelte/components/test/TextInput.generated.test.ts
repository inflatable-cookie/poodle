...
describe("TextInput (svelte) — generated definition drives the DOM", () => {
  it("emits the data attributes under the definition's names and values", () => {
    const { container } = render(TextInput, { props: { id: "t1", validationState: "invalid" } });
  ...
  it("renders the anatomy under the definition's part classes", () => {
    const { container } = render(TextInput, {
      props: { id: "t1", prefix: "$", suffix: "/mo", showCharCount: true, maxLength: 10 },
    });
  ...
  it("conditionally renders the clear button, validation indicator and char count", () => {
    const { container } = render(TextInput, {
      props: { id: "t1", type: "search", defaultValue: "query", validationState: "invalid" },
    });
    const root = container.querySelector(partSelector("root")) as HTMLElement;
    ...
    const plain = render(TextInput, { props: { id: "t2" } });
  ...
  it("emits the TXT-16 style props under the definition's names", () => {
    const { container } = render(TextInput, { props: { id: "t1", type: "search", defaultValue: "q" } });
    const root = container.querySelector(partSelector("root")) as HTMLElement;
    ...
    expect(style).toContain(stylePropName("clear-inset-inline-end"));
    expect(style).toContain(stylePropName("trailing-inset-inline-end"));
  });

  it("does not fire intermediate onValueChange during IME composition", async () => {
    const onValueChange = vi.fn();
    const { container } = render(TextInput, {
      props: { id: "t1", onValueChange },
    });
    ...
    // travels through the event, browser-faithfully.
    ...
    // The composition commits once on compositionend — the single commit
    // point for both runtimes (per the card's no-intermediate-fire
    // acceptance line).
  ...
  it("leaves selection to the DOM — the component carries no selection state", async () => {
    const { container, rerender } = render(TextInput, {
      props: { id: "t1", defaultValue: "hello", placeholder: "a" },
    });
    const input = container.querySelector("input") as HTMLInputElement;
    const root = container.querySelector(partSelector("root")) as HTMLElement;
    ...
    await rerender({ id: "t1", placeholder: "b" });
    expect(input.selectionStart).toBe(1);
