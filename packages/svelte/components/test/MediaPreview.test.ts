...
import MediaPreview from "../src/MediaPreview.svelte";
import { asSnippet } from "./snippet";

describe("MediaPreview (svelte)", () => {
  ...
  it("renders the metadata list with the preview metadata label", () => {
    const { container } = render(MediaPreview, {
      props: { title: "Hero", meta: ["1920 x 1080", "245 KB"] },
    });
  ...
  it("prepends thumbnailMeta to the metadata list", () => {
    const { container } = render(MediaPreview, {
      props: { title: "Hero", thumbnailMeta: "PNG", meta: ["245 KB"] },
    });
    const list = container.querySelector(".poodle-media-preview__meta") as HTMLElement;
  ...
  it("renders custom body content", () => {
    const { container } = render(MediaPreview, {
      props: { title: "Hero", children: asSnippet(() => "<p>body</p>") },
    });
