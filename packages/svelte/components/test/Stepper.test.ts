...
describe("Stepper (svelte)", () => {
  it("falls back to the first step as current and exposes status in accessible names", () => {
    ...
    const labels = [...container.querySelectorAll<HTMLButtonElement>(".poodle-stepper__trigger")].map(
  ...
  it("selects a step on click and reports the change", async () => {
    const onValueChange = vi.fn();
    const { container } = render(Stepper, { props: { steps, onValueChange } });
    const triggers = [...container.querySelectorAll<HTMLButtonElement>(".poodle-stepper__trigger")];

    await fireEvent.click(triggers[2]);
  ...
  it("skips disabled steps when selecting", async () => {
    const onValueChange = vi.fn();
  ...
  it("shows a rerun trigger for complete steps when onRerun is provided", () => {
    const onRerun = vi.fn();
    const { container } = render(Stepper, { props: { steps, onRerun } });
  ...
  it("keeps collapse vertical-only and exposes the collapse summary there", async () => {
    const onCollapsedChange = vi.fn();
    const horizontal = render(Stepper, { props: { steps, collapsible: true } });
    expect(horizontal.container.querySelector(".poodle-stepper__summary")).toBeNull();

    const vertical = render(Stepper, {
      props: { steps, collapsible: true, orientation: "vertical", onCollapsedChange },
    });
