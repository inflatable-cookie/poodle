...
/**
 * Card inactive surfaces (g18.004). The fill lives on the item wrapper so a
 * closable tab's close button sits inside the card. happy-dom cannot resolve
 * recipe var() chains, so the CSS half is the shipped declaration; core's
 * cascade suite computes the winning background.
 */
...
describe("Tabs card item surfaces (svelte)", () => {
  ...
  it("disabled inactive cards still render as item wrappers under the card variant", () => {
    const { container } = render(Tabs, {
      props: { items, defaultValue: "mix", variant: "card" },
    });
    ...
    expect(master!.querySelector<HTMLButtonElement>(".poodle-tabs__tab")!.disabled).toBe(true);
  });
});
