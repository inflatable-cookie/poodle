...
import ListCardCounter from "../src/ListCardCounter.svelte";

describe("ListCardCounter (svelte)", () => {
  it("renders the count and the icon in a span by default", () => {
    const { container } = render(ListCardCounter, { props: { icon: "mail", count: 12 } });
  ...
  it("renders an anchor when href is set", () => {
    const { container } = render(ListCardCounter, {
      props: { icon: "mail", count: 12, href: "/threads/1" },
    });
    const root = container.querySelector(".poodle-list-card-counter") as HTMLElement;
  ...
  it("emits onClick for a linked counter only", async () => {
    const onClick = vi.fn();
    const { container } = render(ListCardCounter, {
      props: { icon: "mail", count: 12, href: "/threads/1", onClick },
    });
    ...
    const plain = render(ListCardCounter, {
      props: { icon: "mail", count: 12, onClick },
    });
    await fireEvent.click(plain.container.querySelector(".poodle-list-card-counter") as HTMLElement);
    expect(onClick).toHaveBeenCalledTimes(1);
  });

  it("wraps the body in a tooltip when tooltip is set", () => {
    const { container } = render(ListCardCounter, {
      props: { icon: "mail", count: 12, tooltip: "Comments" },
    });
