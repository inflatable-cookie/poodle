...
import PageLoading from "../src/PageLoading.svelte";

describe("PageLoading (svelte)", () => {
  it("renders a polite status region with the default label", () => {
    const { container } = render(PageLoading, { props: { visible: true } });
  ...
  it("renders the progress bar when a value is provided and omits it when indeterminate", () => {
    const determinate = render(PageLoading, { props: { value: 40, max: 100 } });
    expect(determinate.container.querySelector(".poodle-page-loading__progress")).not.toBeNull();

    const indeterminate = render(PageLoading, { props: { value: null } });
    expect(indeterminate.container.querySelector(".poodle-page-loading__progress")).toBeNull();
  });

  it("renders the message and the cancel button when enabled", async () => {
    const onCancel = vi.fn();
    const { container } = render(PageLoading, {
      props: { message: "Uploading files...", canCancel: true, onCancel },
    });
  ...
  it("omits the cancel button unless canCancel is true", () => {
    const { container } = render(PageLoading, { props: { message: "Uploading..." } });
