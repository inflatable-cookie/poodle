...
import NavCard from "../src/NavCard.svelte";

describe("NavCard (svelte)", () => {
  it("renders an anchor when href is set and a button otherwise", () => {
    const anchor = render(NavCard, { props: { title: "Docs", href: "/docs" } });
    ...
    const button = render(NavCard, { props: { title: "Docs" } });
  ...
  it("defaults the accessible name to the title and renders badge and description", () => {
    const { container } = render(NavCard, {
      props: { title: "Settings", badge: "New", description: "Tune preferences" },
    });
  ...
  it("emits onClick and swallows clicks while disabled", async () => {
    const onClick = vi.fn();
    const { container } = render(NavCard, { props: { title: "Docs", onClick } });
    await fireEvent.click(container.querySelector(".poodle-nav-card") as HTMLElement);
    expect(onClick).toHaveBeenCalledTimes(1);

    const disabled = render(NavCard, {
      props: { title: "Docs", disabled: true, onClick },
    });
    ...
    await fireEvent.click(disabledEl);
    expect(onClick).toHaveBeenCalledTimes(1);
  });
});
