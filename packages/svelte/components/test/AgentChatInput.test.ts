...
// The composer's whole contract is submit gating and status semantics: an empty
// editor must not submit, Enter/Shift+Enter must split, and `busy` must route to
// stop instead of submit — never both.
describe("AgentChatInput (svelte)", () => {
  ...
  it("disables the action while the editor is empty and enables it with text", () => {
    expect(actionOf(render(AgentChatInput, { props: { value: "" } }).container).disabled).toBe(true);
    expect(actionOf(render(AgentChatInput, { props: { value: "   " } }).container).disabled).toBe(
      true,
    );
    expect(actionOf(render(AgentChatInput, { props: { value: "hi" } }).container).disabled).toBe(
      false,
    );
    expect(
      actionOf(render(AgentChatInput, { props: { value: "", allowEmptySubmit: true } }).container)
        .disabled,
    ).toBe(false);
  });
  ...
  it("submits on Enter and inserts a newline on Shift+Enter", async () => {
    ...
    const { container } = render(AgentChatInput, { props: { value: "ship it", onSubmit } });
  ...
  it("only submits on Cmd/Ctrl+Enter when submitOnEnter is false", async () => {
    const onSubmit = vi.fn();
    const { container } = render(AgentChatInput, {
      props: { value: "ship it", submitOnEnter: false, onSubmit },
    });
    const editor = editorOf(container);
  ...
  it("routes the action and Escape to stop while busy, never to submit", async () => {
    const onSubmit = vi.fn();
    const onStop = vi.fn();
    const { container } = render(AgentChatInput, {
      props: { value: "ship it", status: "busy", onSubmit, onStop },
    });
    ...
    await fireEvent.keyDown(editorOf(container), { key: "Enter" });
    await fireEvent.keyDown(editorOf(container), { key: "Escape" });
  ...
  it("renders an image attachment as a thumbnail tile, files as chips", () => {
    ...
    expect(
      items[1].querySelector(".poodle-agent-chat-input__attachment-label")?.textContent,
    ).toBe("notes.md");

    // Both keep a removal control with the same accessible name shape.
    expect(
      container.querySelectorAll('[aria-label^="Remove "]'),
    ).toHaveLength(2);
  });

  it("renders the context ring only when a limit is supplied", () => {
    const without = render(AgentChatInput, { props: { value: "" } }).container;
    expect(without.querySelector(".poodle-agent-chat-input__context")).toBeNull();

    const { container } = render(AgentChatInput, {
      props: { value: "", contextUsed: 180_000, contextLimit: 200_000 },
    });
  ...
  it("does not clear the editor on submit", async () => {
    const { container } = render(AgentChatInput, { props: { value: "keep me" } });
    await fireEvent.click(actionOf(container));
