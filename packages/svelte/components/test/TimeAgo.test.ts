...
import TimeAgo from "../src/TimeAgo.svelte";

describe("TimeAgo (svelte)", () => {
  it("can place the future direction after the duration", () => {
    ...
    const { container } = render(TimeAgo, {
      props: { datetime, live: false, futureFormat: "from-now" },
    });
  ...
  it("keeps the standalone future form as the default", () => {
    const datetime = Date.now() + 5 * 60_000 + 30_000;
    const { container } = render(TimeAgo, { props: { datetime, live: false } });
