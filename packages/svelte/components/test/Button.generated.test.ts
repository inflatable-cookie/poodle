...
describe("Button (svelte) — generated definition drives the DOM", () => {
  it("emits data attributes under the definition's names and values", () => {
    const { getByRole } = render(Button, {
      props: { variant: "primary", tone: "danger", truncate: true },
    });
    ...
    expect(el.getAttribute(attributeName("truncate"))).not.toBeNull();
  ...
  it("renders the anatomy under the definition's part classes", () => {
    const { getByRole } = render(Button, {
      props: { loading: true, leadingIcon: "plus", trailingIcon: "check" },
    });
    const el = getByRole("button") as HTMLButtonElement;
