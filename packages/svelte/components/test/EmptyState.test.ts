...
describe("EmptyState (svelte)", () => {
  it("labels the section from ariaLabel, falling back to the title", () => {
    const { container } = render(EmptyState, { props: { title: "No projects yet" } });
    ...
    const labelled = render(EmptyState, {
      props: { title: "No projects", ariaLabel: "Empty projects" },
    });
  ...
  it("renders the title as an h3 and the message when given", () => {
    const { container } = render(EmptyState, {
      props: { title: "Nothing here", message: "Create something." },
    });
  ...
  it("projects the variant and size data attributes", () => {
    const { container } = render(EmptyState, {
      props: { title: "No results", variant: "search", size: "compact" },
    });
    const root = container.querySelector(".poodle-empty-state") as HTMLElement;
  ...
  it("keeps the visual region decorative", () => {
    const { container } = render(EmptyState, { props: { title: "No results", variant: "search" } });
  ...
  it("renders the actions region only when an actions snippet is present", () => {
    const withActions = render(EmptyState, {
      props: { title: "No projects", actions: asSnippet(() => "<button>Create</button>") },
    });
    expect(withActions.container.querySelector(".poodle-empty-state__actions")).not.toBeNull();

    const withoutActions = render(EmptyState, { props: { title: "No projects" } });
