...
import PickerShell from "../src/PickerShell.svelte";
import { asSnippet } from "./snippet";

describe("PickerShell (svelte)", () => {
  it("renders the section with title, description, and counts", () => {
    const { container } = render(PickerShell, {
      props: { title: "Select a component", description: "Browse components.", resultCount: 12, selectionCount: 2 },
    });
  ...
  it("renders the body only in the ready state", () => {
    const ready = render(PickerShell, {
      props: { title: "Pick", state: "ready", children: asSnippet(() => "<div>candidates</div>") },
    });
    ...
    const loading = render(PickerShell, { props: { title: "Pick", state: "loading" } });
  ...
  it("shows the spinner in the loading state fallback", () => {
    const { container } = render(PickerShell, { props: { title: "Pick", state: "loading" } });
  ...
  it("renders the sr-only status live region when statusText is provided", () => {
    const { container } = render(PickerShell, {
      props: { title: "Pick", statusText: "3 candidates available", statusId: "picker-status" },
    });
  ...
  it("renders the toolbar, selection, and footer regions when snippets are provided", () => {
    const { container } = render(PickerShell, {
      props: {
        title: "Pick",
        toolbar: asSnippet(() => "<input>"),
        selection: asSnippet(() => "<div>chips</div>"),
        footer: asSnippet(() => "<button>Confirm</button>"),
      },
    });
