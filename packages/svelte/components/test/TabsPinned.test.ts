...
describe("Tabs pinned (svelte)", () => {
  it("pins stay first and last with partition markers", () => {
    ...
    expect(tabs().map((tab) => tab.textContent?.trim())).toEqual(["Home", "Mix", "Master", "Logs"]);
  });

  it("a pinned tab cannot be picked up with Alt+Arrow", async () => {
    const onReorder = vi.fn();
    render(Tabs, { props: { items, defaultValue: "home", reorderable: true, onReorder } });
  ...
  it("an unpinned tab cannot cross into or over a pinned partition", async () => {
    const onReorder = vi.fn();
    render(Tabs, { props: { items, defaultValue: "mix", reorderable: true, onReorder } });
