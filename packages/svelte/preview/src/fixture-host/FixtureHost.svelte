<script lang="ts">
  /**
   * g15.047 — capture-only Button fixture host. The catalogue is preview
   * chrome; this page is the exact scene the Playwright harness
   ...
   * full-viewport `[data-fixture-error]` the harness fails on.
   */
...
`;
  ...
  /**
   * Flat rather than a discriminated union: svelte-check (tsgo through
   * svelte2tsx) does not narrow a boolean-discriminant union in this file,
   * while `FixtureConfig | null` narrows everywhere it matters.
   */
  type ParsedParams = { config: FixtureConfig | null; problems: string[] };
  ...
  const { config, problems } = parseFixtureParams(new URLSearchParams(window.location.search));

  // Theme attributes land synchronously during component init — before the
  // first paint — so the Button's first computed style is already the themed
  // one and no unthemed→themed transition can ever start. (Post-paint
  // application measurably raced the harness: getComputedStyle caught
  // mid-transition colors while the screenshot caught the settled frame.)
  if (config !== null) {
...
{#if config}
  <div class="fixture-host" data-fixture-host data-fixture-ready={ready ? "" : undefined}>
    {#if config.contentKind === "icon-only"}
      <Button
...
{:else}
  <div class="fixture-error" data-fixture-error>{problems.join("; ")}</div>
{/if}
