<script lang="ts">
  import { workstationComponents, findComponent } from "../component-registry";
  ...
  $: entry = activeComponent ? findComponent(activeComponent, "workstation") : undefined;
  $: specimen = entry?.slug ? specimenMap[entry.slug] ?? null : null;
</script>

<div class="catalogue-layout">
  <nav class="catalogue-sidebar" aria-label="Workstation components">
    <ul class="catalogue-sidebar__list">
      {#each workstationComponents as component (component.slug)}
        <li>
          <a
            class="catalogue-sidebar__link"
            class:catalogue-sidebar__link--active={activeComponent === component.slug}
            href="#workstation/{component.slug}"
          >
  ...
  <div class="catalogue-content">
    ...
    {:else}
      <CatalogueLanding tier="workstation" components={workstationComponents} />
    {/if}
  </div>
</div>
