<script lang="ts">
  import { compositeComponents, findComponent } from "../component-registry";
  ...
  $: entry = activeComponent ? findComponent(activeComponent, "composite") : undefined;
  $: specimen = entry?.slug ? specimenMap[entry.slug] ?? null : null;
</script>

<div class="catalogue-layout">
  <nav class="catalogue-sidebar" aria-label="Composite components">
    <ul class="catalogue-sidebar__list">
      {#each compositeComponents as component (component.slug)}
        <li>
          <a
            class="catalogue-sidebar__link"
            class:catalogue-sidebar__link--active={activeComponent === component.slug}
            href="#composites/{component.slug}"
          >
  ...
  <div class="catalogue-content">
    ...
    {:else}
      <CatalogueLanding tier="composite" components={compositeComponents} />
    {/if}
  </div>
</div>
