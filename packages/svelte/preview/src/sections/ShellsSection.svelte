<script lang="ts">
  import { shellComponents, findComponent } from "../component-registry";
  ...
  $: entry = activeComponent ? findComponent(activeComponent, "shell") : undefined;
  $: specimen = entry?.slug ? specimenMap[entry.slug] ?? null : null;
</script>

<div class="catalogue-layout">
  <nav class="catalogue-sidebar" aria-label="Shell components">
    <ul class="catalogue-sidebar__list">
      {#each shellComponents as component (component.slug)}
        <li>
          <a
            class="catalogue-sidebar__link"
            class:catalogue-sidebar__link--active={activeComponent === component.slug}
            href="#shells/{component.slug}"
          >
  ...
  <div class="catalogue-content">
    ...
    {:else}
      <CatalogueLanding tier="shell" components={shellComponents} />
    {/if}
  </div>
</div>
