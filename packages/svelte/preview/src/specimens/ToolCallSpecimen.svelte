...
<SpecimenLayout>
  <SpecimenGroup label="Status" description="Success, running and failure. Only the label takes the danger colour — the detail is already the dimmest thing in the row.">
  ...
  <SpecimenGroup label="Kinds" description="The icon is derived from the label, with an override for hosts using their own vocabulary.">
  ...
  <SpecimenGroup label="Truncation" description="A long detail ellipsises rather than pushing the status indicator out of the row. The accessible name keeps the whole command.">
    <div style="max-width: 26rem;">
          <ToolCall id="long" label="Ran command" detail={longDetail} />
    </div>
  </SpecimenGroup>

  <SpecimenGroup label="Output" description="A row with no output is not a disabled button — it is not in the tab order at all.">
