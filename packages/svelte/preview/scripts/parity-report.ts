...
export function writeParityReport(options: ReportOptions): void {
  validateParityState();

  const repoRoot = path.resolve(options.previewDir, "../../..");
  const gpuiCrossRuntimeParityPath = path.join(
    repoRoot,
    "packages",
    "gpui",
  ...
  const report = {
    ...
    ...(options.sharedContractNote === undefined
      ? {}
      : { sharedContractNote: options.sharedContractNote }),
  ...
  const artifactPath = path.join(options.previewDir, "artifacts", "parity-report.json");
