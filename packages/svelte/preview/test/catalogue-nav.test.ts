...
describe("Svelte catalogue navigation", () => {
  ...
  it("finds the web editor specimens by search and direct route", async () => {
    const search = render(ComponentsSection, { props: { search: "CodeEditor" } });
    expect(
      document.querySelector('[data-catalogue-result="code-editor"]')?.getAttribute("href"),
    ).toBe("#components/code-editor");
    search.unmount();

    const richSearch = render(ComponentsSection, { props: { search: "RichTextEditor" } });
    expect(
      document.querySelector('[data-catalogue-result="rich-text-editor"]')?.getAttribute("href"),
    ).toBe("#components/rich-text-editor");
    richSearch.unmount();

    const rendererSearch = render(ComponentsSection, {
      props: { search: "RichTextRenderer" },
    });
    expect(
    ...
    render(ComponentsSection, {
      props: { activeComponent: "code-editor", search: "" },
    });
    expect(
      document.querySelector(
