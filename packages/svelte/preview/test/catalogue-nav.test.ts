...
describe("Svelte catalogue navigation", () => {
  ...
  it("discloses the active family on a direct component route", () => {
    render(ComponentsSection, { props: { activeComponent: "agent-plan", search: "" } });
    expect(
      document.querySelector('.poodle-catalogue-sidebar [data-catalogue-family="agent-tools"][data-open="true"]'),
    ).toBeTruthy();
    const active = document.querySelector('.poodle-catalogue-sidebar a[href="#components/agent-plan"]');
