...
pub fn keyboard(spec: &KeyboardSpec, theme: &dyn ThemeProvider) -> Node {
    ...
    for key in &state.keys {
        ...
        node.style.descriptor.background = Some(theme.resolve_color(if held { "color.accent.base" } else if key.black { "#131a22" } else { "#f7fafd" }));
...
pub fn mod_matrix_grid(spec: &ModMatrixGridSpec, theme: &dyn ThemeProvider) -> Node {
    ...
    let mut corner = Node::text("Source"); corner.style.descriptor.layout.width = LayoutSizing::Fixed(cell_width * 1.8); header = header.child(corner);
    for destination in &state.destinations { let mut label = Node::text(destination.label.clone()); label.style.descriptor.layout.width = LayoutSizing::Fixed(cell_width); header = header.child(label); }
    ...
    for source in &state.sources {
        ...
        let mut label = Node::text(source.label.clone()); label.style.descriptor.layout.width = LayoutSizing::Fixed(cell_width * 1.8); row = row.child(label);
        for cell in state.cells.iter().filter(|cell| cell.cell.source_id == source.id) {
            ...
            let mut node = Node::container(); node.style.descriptor.layout.width = LayoutSizing::Fixed(cell_width); node.style.descriptor.layout.height = LayoutSizing::Fixed(cell_height); node.style.descriptor.background = Some(theme.resolve_color(if cell.cell.enabled { "color.background.elevated" } else { "color.background.canvas" })); node.style.descriptor.border.width = if cell.focused { 2.0 } else { 1.0 }; node.style.descriptor.border.color = theme.resolve_color("color.border.default"); node.a11y.role = Some(NodeRole::Cell); node.a11y.label = Some(format!("{} to {}, {}, range {} to {}", source.label, destination_label, cell.cell.amount, cell.cell.parameters.min, cell.cell.parameters.max)); node.interaction.focusable = state.enabled;
            let mut zero = Node::container(); zero.style.descriptor.layout.width = LayoutSizing::Fixed(1.0); zero.style.descriptor.layout.height = LayoutSizing::Fixed(cell_height * 0.64); zero.style.descriptor.background = Some(theme.resolve_color("color.border.default")); absolute(&mut zero, cell.zero_norm as f32 * cell_width, cell_height * 0.18); node = node.child(zero);
            let amount_width = cell.fill_span_norm as f32 * cell_width;
            if amount_width > 0.0 { let mut amount = Node::container(); amount.style.descriptor.layout.width = LayoutSizing::Fixed(amount_width); amount.style.descriptor.layout.height = LayoutSizing::Fixed(density_metric(spec.density, [2.0, 4.0, 6.0])); amount.style.descriptor.background = Some(theme.resolve_color(if cell.amount_norm < cell.zero_norm { "color.status.danger" } else { "color.accent.base" })); absolute(&mut amount, cell.fill_start_norm as f32 * cell_width, cell_height / 2.0 - 2.0); node = node.child(amount); }
            row = row.child(node);
        }
        root = root.child(row);
    }
