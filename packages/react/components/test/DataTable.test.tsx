...
describe("DataTable (react)", () => {
  it("renders rows and reports the next sort direction from the controlled state", () => {
    const onSortChange = vi.fn();
    const { container } = render(<DataTable columns={columns} rows={rows} onSortChange={onSortChange} />);
    ...
    const { container: descending } = render(
      <DataTable columns={columns} rows={rows} sortColumnId="name" sortDirection="asc" onSortChange={onSortChange} />,
    );
    const activeSortButton = descending.querySelector(".poodle-data-table__sort") as HTMLButtonElement;
  ...
  it("projects aria-sort on the active sort column", () => {
    const { container } = render(
      <DataTable columns={columns} rows={rows} sortColumnId="role" sortDirection="desc" />,
    );
  ...
  it("reports row toggles and select-all from the selection column", () => {
    const onRowToggle = vi.fn();
    const onToggleAll = vi.fn();
    const { container } = render(
      <DataTable columns={columns} rows={rows} selectable onRowToggle={onRowToggle} onToggleAll={onToggleAll} />,
    );
  ...
  it("marks selected rows with aria-selected", () => {
    const { container } = render(
      <DataTable columns={columns} rows={rows} selectable selectedRowIds={["r2"]} />,
    );
  ...
  it("shows the empty message when no rows are present", () => {
    const { container } = render(
      <DataTable columns={columns} rows={[]} emptyMessage="No members match" />,
    );
  ...
  it("renders skeleton rows while loading with no rows", () => {
    const { container } = render(
      <DataTable columns={columns} rows={[]} loading loadingRows={3} />,
    );
    expect(container.querySelectorAll("tbody tr.poodle-data-table__loading-row").length).toBe(3);
  });

  it("renders the pagination footer and reports page changes", () => {
  ...
  it("exports CSV through the toolbar and reports the filename", async () => {
    ...
    const { container } = render(
      <DataTable columns={columns} rows={rows} showExport exportFilename="team.csv" onExportCsv={onExportCsv} />,
    );
  ...
  it("excludes hidden columns from rendering and from the CSV export", async () => {
    const onExportCsv = vi.fn();
    const { container } = render(
      <DataTable columns={columns} rows={rows} hiddenColumnIds={["role"]} showExport onExportCsv={onExportCsv} />,
    );
  ...
  it("renders the filter row for filterable columns", () => {
    ...
    const { container } = render(<DataTable columns={filterable} rows={rows} />);
