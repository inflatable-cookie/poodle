...
describe("CommandPalette (react)", () => {
  it("renders nothing while closed", () => {
    const { container } = render(<CommandPalette open={false} items={items} />);
  ...
  it("renders a modal dialog with title, query, and status when open", () => {
    const { container } = render(
      <CommandPalette open items={items} title="Commands" description="Run an action" />,
    );
  ...
  it("reports the query changes and command selection", async () => {
    ...
    const { container } = render(
      <CommandPalette open items={items} onQueryChange={onQueryChange} onCommandSelect={onCommandSelect} />,
    );

    const query = container.querySelector("#command-palette-query") as HTMLInputElement;
    fireEvent.input(query, { target: { value: "save" } });
    ...
    fireEvent.click(saveOption.querySelector(".poodle-list-card") as HTMLElement);
  ...
  it("reports the no-results status copy when the host sets the no-results state", () => {
    const { container } = render(<CommandPalette open items={items} query="zzz" state="no-results" />);
    const status = container.querySelector("#command-palette-status") as HTMLElement;
    expect(status.textContent).toContain('No commands match "zzz".');
  });

  it("closes via Escape and the close button", () => {
    const onOpenChange = vi.fn();
    const { container } = render(
      <CommandPalette open items={items} onOpenChange={onOpenChange} />,
    );
  ...
  it("selects the active command on Enter", async () => {
    const onCommandSelect = vi.fn();
    const { container } = render(
      <CommandPalette open items={items} onCommandSelect={onCommandSelect} />,
    );
    await waitFor(() => {
      expect(container.querySelector('[role="option"]')).not.toBeNull();
    });
  ...
  it("renders the invocation hint when provided", () => {
    const { container } = render(
      <CommandPalette open items={items} invocationHint="Cmd+K" />,
    );
