...
describe("ToggleGroup (react)", () => {
  ...
  it("clears a single-mode selection to null only when allowDeactivation is set", () => {
    ...
    fireEvent.click(items(without.container)[0]);
    expect(onValueChange).toHaveBeenCalledWith("grid");
  });

  it("projects one selected or first-enabled tab stop in single mode", () => {
    const selected = render(<ToggleGroup options={options} value="list" />);
    expect(items(selected.container).map((item) => item.tabIndex)).toEqual([-1, 0, -1]);

    const first = render(<ToggleGroup options={options} />);
    expect(items(first.container).map((item) => item.tabIndex)).toEqual([0, -1, -1]);

    const skipped = render(<ToggleGroup options={skipOptions} value="list" />);
    expect(items(skipped.container).map((item) => item.tabIndex)).toEqual([0, -1, -1]);
  });

  it("moves selection with Left/Right, wrapping and skipping disabled options", () => {
    const onValueChange = vi.fn();
    const { container } = render(
      <ToggleGroup options={skipOptions} value="grid" onValueChange={onValueChange} />,
    );
  ...
  it("keeps multiple mode as ordinary buttons and ignores arrows", () => {
    const onValueChange = vi.fn();
    const { container } = render(
      <ToggleGroup options={options} value={["grid"]} selectionMode="multiple" onValueChange={onValueChange} />,
    );
    const buttons = items(container);
  ...
  it("keeps two same-valued groups independent", () => {
    const leftChange = vi.fn();
    const rightChange = vi.fn();
    const { container } = render(
      <>
        <ToggleGroup options={options} value="grid" onValueChange={leftChange} />
        <ToggleGroup options={options} value="grid" onValueChange={rightChange} />
      </>,
    );
    const groups = Array.from(container.querySelectorAll(".poodle-toggle-group"));
    const leftItems = items(groups[0] as HTMLElement);
    const rightItems = items(groups[1] as HTMLElement);

    fireEvent.keyDown(leftItems[0], { key: "ArrowRight" });
