...
import { ActionDiscoveryPanel } from "../src/ActionDiscoveryPanel";
...
describe("ActionDiscoveryPanel (react)", () => {
  it("renders a labelled listbox with grouped actions", () => {
    const { container } = render(<ActionDiscoveryPanel items={items} ariaLabel="Command results" />);
  ...
  it("marks the active item with aria-selected", () => {
    const { container } = render(<ActionDiscoveryPanel items={items} activeId="save" />);
    const options = [...container.querySelectorAll('[role="option"]')];
  ...
  it("renders shortcuts and badges on their rows", () => {
    const { container } = render(<ActionDiscoveryPanel items={items} />);
  ...
  it("reports item selection on click", () => {
    const onItemSelect = vi.fn();
    const { container } = render(<ActionDiscoveryPanel items={items} onItemSelect={onItemSelect} />);
    const saveRow = [...container.querySelectorAll('[role="option"]')].find((option) =>
      option.textContent?.includes("Save"),
    ) as HTMLElement;
    fireEvent.click(saveRow.querySelector(".poodle-list-card") as HTMLElement);
    expect(onItemSelect).toHaveBeenCalledWith("save");
  });

  it("renders five skeleton rows while loading", () => {
    const { container } = render(<ActionDiscoveryPanel items={items} state="loading" />);
  ...
  it("renders contextual empty states for error, empty, and no-results", () => {
    const error = render(<ActionDiscoveryPanel items={[]} state="error" />);
    expect(error.container.textContent).toContain("Could not load actions");

    const empty = render(<ActionDiscoveryPanel items={[]} state="empty" />);
    expect(empty.container.textContent).toContain("No actions available");

    const noResults = render(<ActionDiscoveryPanel items={[]} state="no-results" />);
