...
describe("CodeEditor (react)", () => {
  ...
  it("prop updates replace the text without emitting a callback", async () => {
    ...
    const view = render(<CodeEditor value="one" onChange={onChange} />);
    await vi.waitFor(() => {
  ...
  it("a host revert restores the text without a second callback", async () => {
    const onChange = vi.fn();
    const view = render(<CodeEditor value="draft" onChange={onChange} />);
    await vi.waitFor(() => {
      expect(visibleText(view.container)).toBe("draft");
    });
    view.rerender(<CodeEditor value="edited by host" onChange={onChange} />);
    view.rerender(<CodeEditor value="draft" onChange={onChange} />);
    await vi.waitFor(() => {
      expect(visibleText(view.container)).toBe("draft");
    });
    expect(onChange).not.toHaveBeenCalled();
  ...
  it("read-only keeps selection while refusing mutation affordances", async () => {
    const { container } = render(<CodeEditor value="held" readOnly />);
    await vi.waitFor(() => {
  ...
  it("disabled leaves the focus order and announces itself", async () => {
    const { container } = render(<CodeEditor value="held" disabled />);
    await vi.waitFor(() => {
  ...
  it("unmount destroys the engine view", async () => {
    const { container, unmount } = render(<CodeEditor value="bye" />);
    await vi.waitFor(() => {
...
describe("CodeEditor keyboard escape (react)", () => {
  ...
  it("Tab indents in indent mode with one exact change", async () => {
    const onChange = vi.fn();
    const { container } = render(
      <CodeEditor value="one" tabBehavior="indent" onChange={onChange} />,
    );
    await vi.waitFor(() => {
      expect(contentOf(container).textContent).toContain("one");
    });
...
describe("CodeEditor diagnostic navigation (react)", () => {
  it("F8 walks diagnostics and announces severity with line:column", async () => {
    const { container } = render(
      <CodeEditor value={DIAGNOSTIC_VALUE} diagnostics={DIAGNOSTICS} />,
    );
    await vi.waitFor(() => {
      expect(contentOf(container).textContent).toContain("one");
    });
  ...
  it("an unrelated re-render keeps the F8 active diagnostic", async () => {
    const view = render(<CodeEditor value={DIAGNOSTIC_VALUE} diagnostics={DIAGNOSTICS} />);
    await vi.waitFor(() => {
    ...
    view.rerender(
      <CodeEditor value={DIAGNOSTIC_VALUE} diagnostics={DIAGNOSTICS} placeholder="touched" />,
    );
    await vi.waitFor(() => {
      expect(diagnosticMessage(view.container)).toBe("error 1:1 — first");
    });
...
describe("CodeEditor search (react)", () => {
...
describe("CodeEditor user transactions (react)", () => {
  it("Enter, undo, and redo each emit one exact change", async () => {
    const onChange = vi.fn();
    const { container } = render(<CodeEditor value="ab" onChange={onChange} />);
    await vi.waitFor(() => {
 ...
 * active diagnostics.
 */
describe("CodeEditor line-number reconfiguration (react)", () => {
  ...
  async function mountedOn(extra: Record<string, unknown> = {}) {
    const view = render(<CodeEditor value={"one\ntwo"} {...(extra as object)} />);
    await vi.waitFor(() => {
  ...
  it("a live false removes the gutter and a live true restores it without a new editor", async () => {
    ...
    expect(editor).not.toBeNull();
    view.rerender(<CodeEditor value={"one\ntwo"} lineNumbers={false} />);
    await vi.waitFor(() => {
  ...
  it("value, selection, focus, and undo history survive both transitions", async () => {
    ...
    press(content, "End");
    view.rerender(<CodeEditor value={"\none\ntwo"} lineNumbers={false} onChange={onChange} />);
    await vi.waitFor(() => {
      expect(lineGutter(view.container)).toBeNull();
    });
    expect(document.activeElement).toBe(content);
    view.rerender(<CodeEditor value={"\none\ntwo"} lineNumbers={true} onChange={onChange} />);
    await vi.waitFor(() => {
      expect(lineGutter(view.container)).not.toBeNull();
    });
    expect(document.activeElement).toBe(content);
  ...
  it("an active diagnostic and F8 navigation survive the toggle", async () => {
    ...
    view.rerender(
      <CodeEditor value={DIAGNOSTIC_VALUE} diagnostics={DIAGNOSTICS} lineNumbers={false} />,
    );
    await vi.waitFor(() => {
      expect(lineGutter(view.container)).toBeNull();
    });
    ...
    view.rerender(
      <CodeEditor value={DIAGNOSTIC_VALUE} diagnostics={DIAGNOSTICS} lineNumbers={true} />,
    );
    await vi.waitFor(() => {
      expect(lineGutter(view.container)).not.toBeNull();
    });
