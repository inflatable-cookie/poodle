...
import { MetricTile } from "../src/MetricTile";

describe("MetricTile (react)", () => {
  it("computes the accessible name from label and value", () => {
    const { container } = render(<MetricTile label="Coverage" value="94%" />);
  ...
  it("honours an explicit ariaLabel override", () => {
    const { container } = render(
      <MetricTile label="Coverage" value="94%" ariaLabel="Coverage percent" />,
    );
    const root = container.querySelector(".poodle-state-tile") as HTMLElement;
  ...
  it("renders a decorative sparkline from 2+ data points", () => {
    const { container } = render(
      <MetricTile label="Requests" value="1,204" sparklineData={[800, 920, 850, 1100]} />,
    );
  ...
  it("omits the sparkline with fewer than two data points", () => {
    const { container } = render(<MetricTile label="Requests" value="10" sparklineData={[10]} />);
