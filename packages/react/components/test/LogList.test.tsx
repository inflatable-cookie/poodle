...
describe("LogList (react)", () => {
  it("renders stream mode as a log region with level counts", () => {
    const { container } = render(<LogList entries={streamEntries} />);
  ...
  it("filters stream entries by level when a level chip is activated", () => {
    const { container } = render(<LogList entries={streamEntries} />);
  ...
  it("filters stream entries by text", () => {
    const { container } = render(<LogList entries={streamEntries} />);
    const search = container.querySelector(
      'input[aria-label="Filter log messages"]',
    ) as HTMLInputElement;
    fireEvent.input(search, { target: { value: "slow" } });
    const messages = [...container.querySelectorAll(".poodle-log-list__msg")].map(
      (el) => el.textContent,
    );
    expect(messages).toEqual(["Slow query detected"]);
  ...
  it("caps stream entries at maxEntries", () => {
    ...
    const { container } = render(<LogList entries={many} maxEntries={3} />);
  ...
  it("auto-detects audit mode from the entry shape", () => {
    const { container } = render(<LogList entries={auditEntries} />);
  ...
  it("links actors when getActorHref is provided", () => {
    const { container } = render(
      <LogList entries={auditEntries} getActorHref={(actor) => `/users/${actor.id}`} />,
    );
  ...
  it("renders audit loading, error, and empty states", () => {
    const loading = render(<LogList entries={[]} variant="audit" loading />);
    expect(loading.container.textContent).toContain("Loading log entries...");

    const error = render(<LogList entries={[]} variant="audit" error="Audit failed" />);
    expect(error.container.querySelector('[role="alert"]')?.textContent).toContain("Audit failed");

    const empty = render(<LogList entries={[]} variant="audit" emptyMessage="Nothing logged" />);
    expect(empty.container.textContent).toContain("Nothing logged");
  });

  it("renders pagination when total exceeds page size and reports page changes", () => {
  ...
  it("shows the clear-filters control only with active filters and a callback", () => {
    ...
    const inactive = render(
      <LogList entries={auditEntries} variant="audit" filters={filters} onClearFilters={onClearFilters} />,
    );
    expect(
