...
describe("AgentPlanRecord (react)", () => {
  const plan = "# Step 1\n\nDo the thing.";

  it("shows the status badge with fallback wording and collapsed summary by default", () => {
    const { container } = render(<AgentPlanRecord plan={plan} status="accepted" />);
  ...
  it("overrides the badge wording and renders the decided-at meta", () => {
    const { container } = render(
      <AgentPlanRecord plan={plan} status="dismissed" decisionLabel="Not now" decidedAt="2026-08-16" />,
    );
  ...
  it("truncates a long plan to the summary budget with an ellipsis in budget", () => {
    ...
    const { container } = render(
      <AgentPlanRecord plan={longPlan} status="accepted" summaryMaxLength={40} />,
    );
  ...
  it("exposes the disclosure state through aria-expanded and the toggle label", () => {
    const collapsed = render(<AgentPlanRecord plan={plan} status="accepted" />);
    ...
    const expanded = render(<AgentPlanRecord plan={plan} status="accepted" expanded />);
