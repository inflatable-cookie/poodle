...
function insertButton(container: HTMLElement): HTMLButtonElement {
  return policy(container).querySelectorAll<HTMLButtonElement>(
...
async function confirmImagePicker(): Promise<void> {
  ...
  act(() => {
    fireEvent.click(document.querySelector("[data-part='image-picker-insert'] button")!);
  });
}

describe("g18.014 rich text image policy specimen proof (react)", () => {
  ...
  it("loads the seeded fixture and admits the insert command when images are on", async () => {
    ...
    expect(policy(container).querySelector("[data-command='insert-image'] button")).not.toBeNull();
    expect(metric(container, "images")).toBe("1");
    expect(metric(container, "requests")).toBe("0");
  });
