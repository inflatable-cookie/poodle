...
import { CodeEditor } from "@inflatable-cookie/poodle-react/editor";
 ...
 */
describe("g18.012 CodeEditor extensible language registry (react)", () => {
  it("the specimen imports the adapter subpath and names its own grammars", () => {
    const source = readFileSync(
      join(PREVIEW_ROOT, "src/gallery/specimens/CodeEditorSpecimen.tsx"),
      "utf8",
    );
    expect(source).toContain('from "@inflatable-cookie/poodle-react/editor/codemirror"');
  ...
  it("a missing id and a failed load refuse instead of falling back", async () => {
    ...
    // Mounted refusal: render throws rather than silently presenting plain
    // text for a language the host asked for.
    let threw = false;
    try {
      render(<CodeEditor value="one" language="cobol" languageRegistry={registry} />);
    } catch (error) {
