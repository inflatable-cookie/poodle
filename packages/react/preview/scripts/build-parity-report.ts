...
} from "../../../svelte/preview/src/parity";
...
function countCoverageKind(harnessName: keyof HarnessCoverage): Record<string, number> {
...
function validateParityState(): void {
  ...
  const docsCatalogSectionIds = new Set(docsNavigationSections.map((entry) => entry.id));
  const seenPackageExports = new Set<string>();

  for (const sectionId of docsSectionIds) {
    if (!parityTargetIds.has(sectionId)) errors.push(`Missing parity target for section "${sectionId}".`);
  }

  for (const target of parityTargets) {
    if (!docsSectionIds.has(target.sectionId)) errors.push(`Parity target "${target.sectionId}" is not a known docs section.`);
    if (target.automatedChecks.length === 0) errors.push(`Parity target "${target.sectionId}" has no automated checks.`);
    ...
    if (target.reviewRoutes.length === 0) errors.push(`Parity target "${target.sectionId}" has no review routes.`);

    const routeUrls = new Set<string>();
    for (const route of target.reviewRoutes) {
      if (route.state.sectionId !== target.sectionId) {
        errors.push(`Parity target "${target.sectionId}" has a route pointing at "${route.state.sectionId}".`);
      }
      const url = buildPreviewUrl(route.state);
      if (routeUrls.has(url)) errors.push(`Parity target "${target.sectionId}" has duplicate review URL "${url}".`);
      routeUrls.add(url);
    }
  }
  ...
  for (const entry of packageSurfaceCoverage) {
    ...
    if (seenPackageExports.has(exportId)) errors.push(`Package surface coverage duplicates "${exportId}".`);
    seenPackageExports.add(exportId);
    if (entry.note.trim().length === 0) errors.push(`Package surface coverage "${exportId}" is missing review guidance.`);
    if (entry.status === "previewed") {
      if (entry.sectionIds.length === 0) errors.push(`Package surface coverage "${exportId}" is previewed without section coverage.`);
      const seenSectionIds = new Set<string>();
      for (const sectionId of entry.sectionIds) {
        if (!docsCatalogSectionIds.has(sectionId)) errors.push(`Package surface coverage "${exportId}" references unknown section "${sectionId}".`);
        if (seenSectionIds.has(sectionId)) errors.push(`Package surface coverage "${exportId}" repeats section "${sectionId}".`);
...
const gpuiCrossRuntimeParity = JSON.parse(fs.readFileSync(gpuiCrossRuntimeParityPath, "utf8")) as {
  ...
  sectionReports: Array<{ sectionId: string; sideBySideReview: boolean; parityMode: string; intentionalDeltaIds: string[] }>;
  deltaRegister: Array<{ id: string; status: string; sectionIds: string[] }>;
  acceptanceHarness: { suiteId: string; status: string; coveredPackages: string[]; evidenceArtifacts: string[] };
};
...
const report = {
  ...
  crossRuntime: {
    ...
    sideBySideSections: gpuiCrossRuntimeParity.sectionReports
      ...
      .map((entry) => ({ sectionId: entry.sectionId, parityMode: entry.parityMode, deltaIds: entry.intentionalDeltaIds })),
    deltaRegister: gpuiCrossRuntimeParity.deltaRegister.map((entry) => ({ id: entry.id, status: entry.status, sectionIds: entry.sectionIds })),
...
console.log(`Wrote parity report with ${report.summary.sectionCount} sections to ${path.relative(process.cwd(), artifactPath)}`);
