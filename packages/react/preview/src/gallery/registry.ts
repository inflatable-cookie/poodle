...
function entry(displayName: string, tag: ComponentTag, description: string, hasSpecimen = false): ComponentEntry {
  return {
...
// Mirror of the Svelte preview's component-registry (same slugs, tags, order)
// so the two galleries route identically for side-by-side comparison. Keep in
// sync when the Svelte registry changes.
export const allComponents: ComponentEntry[] = [
  ...
  entry("AgentMessage", "display", "One turn of agent prose, rendered from the shared markdown block model.", false),
  entry("ToolCallGroup", "display", "A contiguous run of tool calls behind one collapsible summary.", false),
  entry("ToolCall", "display", "One row of agent work, with its output behind a disclosure.", false),
  entry("ChangedFiles", "display", "What a turn touched: counted summary expanding into a directory tree.", false),
  ...
  entry("ListGrid", "layout", "Responsive auto-fill grid for card or tile collections with optional header actions.", true),
  ...
  entry("ErrorBoundary", "feedback", "React error boundary with retryable empty-state fallback.", true),
...
export function findComponent(componentSlug: string): ComponentEntry | undefined {
  return allComponents.find((c) => c.slug === componentSlug);
}
