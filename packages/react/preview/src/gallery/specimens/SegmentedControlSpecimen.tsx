...
export function SegmentedControlSpecimen() {
  ...
  return (
    <SpecimenLayout
      sizes={(size) => (
        <SegmentedControl options={viewOptions} defaultValue="grid" size={size} ariaLabel={size + " view mode"} />
      )}
      densities={(density) => (
        <SegmentedControl options={viewOptions} defaultValue="grid" density={density} ariaLabel={density + " view mode"} />
      )}
    >
