...
pub fn js_media_picker(spec: &MediaPickerSpec, theme: &JetstreamThemeProvider) -> JsEl {
    ...
    let thumb_size = rem_to_px(match effective_size {
        poodle_specs::ControlSize::Xs => 3.5,
        poodle_specs::ControlSize::Sm => 4.25,
        poodle_specs::ControlSize::Md => 4.5,
        poodle_specs::ControlSize::Lg => 5.0,
        poodle_specs::ControlSize::Xl => 5.5,
    });

    // Density-driven spacing from contract
    let (grid_gap, item_pad) = match spec.density {
        poodle_specs::ControlDensity::Compact => (0.25, 0.25),
        poodle_specs::ControlDensity::Default => (0.375, 0.375),
        poodle_specs::ControlDensity::Comfortable => (0.5, 0.5),
    };
