...
//!
//! Contract: `docs/contracts/foundation/date-time-picker.md`
...
use poodle_primitives::DateTimePickerSpec;
...
pub fn js_date_time_picker(spec: &DateTimePickerSpec, theme: &JetstreamThemeProvider) -> JsEl {
    ...
    let display = if has_value {
        let date_part = val.date.as_deref().unwrap_or("");
        let time_part = val.time.as_deref().unwrap_or("");
