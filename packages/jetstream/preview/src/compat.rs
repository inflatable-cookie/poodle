...
pub fn js_spinner(spec: &SpinnerSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::spinner(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_accordion(spec: &AccordionSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::accordion(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_message_center(spec: &MessageCenterSpec, theme: &JetstreamThemeProvider) -> El {
    ...
    El(pr::message_center(spec, &pr::context::RenderContext::new(theme), handlers))
}
...
pub fn js_app_header(spec: &AppHeaderSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::app_header(spec, &pr::context::RenderContext::new(theme), None, None, None, None))
}
...
pub fn js_button(spec: &ButtonSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::button(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_calendar(spec: &CalendarSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::calendar(spec, &pr::context::RenderContext::new(theme), pr::CalendarHandlers::default()))
}

pub fn js_callout(spec: &CallOutSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::callout(spec, &pr::context::RenderContext::new(theme), pr::CalloutHandlers::default()))
}
...
pub fn js_collapsible(
    ...
    El(pr::collapsible(spec, &pr::context::RenderContext::new(theme), content, None))
}
...
pub fn js_command_palette(spec: &CommandPaletteSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::command_palette(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_context_menu(spec: &ContextMenuSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::context_menu(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_data_table_loading(
    ...
    El(pr::data_table_loading(spec, &pr::context::RenderContext::new(theme), row_count))
}
...
pub fn js_debug_dialog(spec: &DebugDialogSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::debug_dialog(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_detail_item(spec: &DetailItemSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::detail_item(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_detail_section(
    ...
    El(pr::detail_section(spec, &pr::context::RenderContext::new(theme), content, actions))
}
...
pub fn js_dialog(
    ...
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    let actions: Option<Node> = actions.map(Node::from);
    El(pr::dialog(spec, &pr::context::RenderContext::new(theme), children, actions, None))
}
...
pub fn js_drawer(spec: &DrawerSpec, theme: &JetstreamThemeProvider, content: Option<El>) -> El {
    let content: Option<Node> = content.map(Node::from);
    El(pr::drawer(spec, &pr::context::RenderContext::new(theme), content, None, None))
}
...
pub fn js_drawer_with_actions(
    ...
    let content: Option<Node> = content.map(Node::from);
    let actions: Option<Node> = actions.map(Node::from);
    El(pr::drawer(spec, &pr::context::RenderContext::new(theme), content, actions, None))
}

pub fn js_duration_input(spec: &DurationInputSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::duration_input(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_editable_label(spec: &EditableLabelSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::editable_label(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_embed_input(spec: &EmbedInputSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::embed_input(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_embed_preview(spec: &EmbedPreviewSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::embed_preview(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_empty_state(spec: &EmptyStateSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::empty_state(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_error_boundary(
    ...
    El(pr::error_boundary(spec, &pr::context::RenderContext::new(theme), child))
}

pub fn js_eyebrow(spec: &EyebrowSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::eyebrow(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_field_set(spec: &FieldSetSpec, theme: &JetstreamThemeProvider, children: Vec<El>) -> El {
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::field_set(spec, &pr::context::RenderContext::new(theme), children))
}

pub fn js_file_upload(spec: &FileUploadSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::file_upload(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_form_actions(
    ...
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::form_actions(spec, &pr::context::RenderContext::new(theme), children))
}
...
pub fn js_form_actions_full(
    ...
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::form_actions_full(spec, &pr::context::RenderContext::new(theme), danger, children))
}
...
pub fn js_form_layout(
    ...
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    let actions: Option<Node> = actions.map(Node::from);
    El(pr::form_layout(spec, &pr::context::RenderContext::new(theme), children, actions))
}
...
pub fn js_form_shell(
    ...
    let section_slots: Vec<Option<Node>> = section_slots
        ...
        .collect();
    El(pr::form_shell(spec, &pr::context::RenderContext::new(theme), section_slots, actions_slot))
}

pub fn js_grid(spec: &GridSpec, theme: &JetstreamThemeProvider, children: Vec<El>) -> El {
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::grid(spec, &pr::context::RenderContext::new(theme), children))
}
...
pub fn js_hover_card(
    ...
    let content: Option<Node> = content.map(Node::from);
    El(pr::hover_card(spec, &pr::context::RenderContext::new(theme), content))
}

pub fn js_icon(spec: &IconSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::icon(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_icon_button(spec: &IconButtonSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::icon_button(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_inline_list_section(
    ...
    let action: Option<Node> = action.map(Node::from);
    El(pr::inline_list_section(spec, &pr::context::RenderContext::new(theme), items, action))
}

pub fn js_inline_remediation(spec: &InlineRemediationSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::inline_remediation(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_list_card_counter(spec: &ListCardCounterSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::list_card_counter(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_list_grid(
    ...
    let header: Option<Node> = header.map(Node::from);
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::list_grid(spec, &pr::context::RenderContext::new(theme), header, children))
}

pub fn js_log_list(spec: &LogListSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::log_list(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_markdown_editor(spec: &MarkdownEditorSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::markdown_editor(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_media_browse_panel(spec: &MediaBrowsePanelSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::media_browse_panel(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_media_preview(spec: &MediaPreviewSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::media_preview(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_media_thumbnail(spec: &MediaThumbnailSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::media_thumbnail(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_menu(spec: &MenuSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::menu(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_menubar(spec: &MenubarSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::menubar(spec, &pr::context::RenderContext::new(theme), None, None))
}

pub fn js_meta_bar(spec: &MetaBarSpec, theme: &JetstreamThemeProvider, children: Vec<El>) -> El {
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::meta_bar(spec, &pr::context::RenderContext::new(theme), children))
}
...
pub fn js_meta_item(spec: &MetaItemSpec, theme: &JetstreamThemeProvider, value: Option<El>) -> El {
    ...
    El(pr::meta_item(spec, &pr::context::RenderContext::new(theme), value))
}

pub fn js_meter(spec: &MeterSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::meter(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_metric_tile(spec: &MetricTileSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::metric_tile(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_model_picker(
    ...
    El(pr::model_picker(spec, &pr::context::RenderContext::new(theme), instance_id, None))
}

pub fn js_nav_card(spec: &NavCardSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::nav_card(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_navigation_menu(spec: &NavigationMenuSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::navigation_menu(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_order_by(spec: &OrderBySpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::order_by(spec, &pr::context::RenderContext::new(theme), pr::OrderByHandlers::default()))
}

pub fn js_page_header(spec: &PageHeaderSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::page_header(spec, &pr::context::RenderContext::new(theme), None, None, None))
}
...
pub fn js_page_loading(spec: &PageLoadingSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::page_loading(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_pagination(spec: &PaginationSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::pagination(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_pagination_summary(spec: &PaginationSummarySpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::pagination_summary(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_password_requirements(
    ...
    El(pr::password_requirements(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_pill(spec: &PillSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::pill(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_popover(spec: &PopoverSpec, theme: &JetstreamThemeProvider, content: Option<El>) -> El {
    let content: Option<Node> = content.map(Node::from);
    El(pr::popover(spec, &pr::context::RenderContext::new(theme), content))
}

pub fn js_progress(spec: &ProgressSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::progress(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_radio_group(spec: &RadioGroupSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::radio_group(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_rating(spec: &RatingSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::rating(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_ref_select(spec: &RefSelectSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::ref_select(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_region(spec: &RegionSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::region(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_resize_handle(spec: &ResizeHandleSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::resize_handle(spec, &pr::context::RenderContext::new(theme), None))
}
...
pub fn js_scroll_shell(
    ...
    let children: Vec<Node> = children.into_iter().map(Node::from).collect();
    El(pr::scroll_shell(spec, &pr::context::RenderContext::new(theme), children))
}

pub fn js_segmented_control(spec: &SegmentedControlSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::segmented_control(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_select(spec: &SelectSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::select(spec, &pr::context::RenderContext::new(theme), &pr::SelectHandlers::default()))
}
...
pub fn js_separator(spec: &SeparatorSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::separator(spec, &pr::context::RenderContext::new(theme)))
}
...
pub fn js_shell_status_bar(
    ...
    El(pr::shell_status_bar(spec, &pr::context::RenderContext::new(theme), leading, trailing))
}

pub fn js_sidebar_nav(spec: &SidebarNavSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::sidebar_nav(spec, &pr::context::RenderContext::new(theme), None))
}

pub fn js_skeleton(spec: &SkeletonSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::skeleton(spec, &pr::context::RenderContext::new(theme)))
}

pub fn js_slider(spec: &SliderSpec, theme: &JetstreamThemeProvider) -> El {
    El(pr::slider(spec, &pr::context::RenderContext::new(theme), &pr::SliderHandlers::default()))
}
