//! Breadcrumbs specimen — breadcrumb navigation with multiple items.

use jetstream_runtime::ui_element::*;
use pug_jetstream::JetstreamThemeProvider;
use pug_jetstream_components::breadcrumbs_comp::js_breadcrumbs;
use pug_jetstream_components::theme_ext::*;
use pug_primitives::{BreadcrumbItem, BreadcrumbsSpec};

pub fn render(theme: &JetstreamThemeProvider) -> JsEl {
    let secondary = resolve_color(theme, "semantic.color.text.secondary");

    let items = vec![
        BreadcrumbItem::new("home", "Home").with_href("/"),
        BreadcrumbItem::new("projects", "Projects").with_href("/projects"),
        BreadcrumbItem::new("current", "Jetstream Preview").with_is_current(true),
    ];

    div().flex_col().gap(24.0)
        .child(group("3-item breadcrumb", secondary,
            js_breadcrumbs(&BreadcrumbsSpec::new(items), theme)
        ))
}

fn group(title: &str, text_secondary: glam::Vec4, content: JsEl) -> JsEl {
    div().flex_col().gap(8.0)
        .child(label(title).text_color(text_secondary).text_size(11.0))
        .child(content)
}
