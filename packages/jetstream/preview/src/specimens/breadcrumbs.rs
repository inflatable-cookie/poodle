    ...
    div()
        ...
        .child(group(
            ...
            div()
                .flex_col()
                .gap(12.0)
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_size(ControlSize::Xs),
                    theme,
                ))
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_size(ControlSize::Sm),
                    theme,
                ))
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_size(ControlSize::Md),
                    theme,
                ))
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_size(ControlSize::Lg),
                    theme,
                ))
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_size(ControlSize::Xl),
                    theme,
                )),
        ))
        ...
        .child(group(
            ...
            div()
                ...
                .gap(12.0)
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_density(ControlDensity::Compact),
                    theme,
                ))
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_density(ControlDensity::Default),
                    theme,
                ))
                .child(js_breadcrumbs(
                    &BreadcrumbsSpec::new(basic_items()).with_density(ControlDensity::Comfortable),
                    theme,
                )),
        ))
}
