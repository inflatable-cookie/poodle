...
pub fn render(theme: &JetstreamThemeProvider) -> JsEl {
    ...
    div().flex_col().gap(24.0)
        ...
        .child(group("Highlighted", secondary,
            div().w(300.0).child(
                js_collapsible(
                    &CollapsibleSpec::new().with_title("Focused section").with_highlighted(true).with_default_open(true),
                    theme,
                    Some(label("Highlighted collapsibles draw attention to a matched section.").text_color(text_muted).text_size(body_font)),
                )
            )
        ))
