...
pub fn render(theme: &JetstreamThemeProvider) -> El {
    ...
    let plain = vec![SidebarNavGroup::new(
        "docs",
        vec![
    ...
    div()
        ...
        .gap(24.0)
        .child(group(
            "Plain list (untitled, active item)",
            secondary,
            js_sidebar_nav(&SidebarNavSpec::new(plain).with_value("components"), theme),
        ))
        .child(group(
            "Grouped, with active item + disabled",
            secondary,
            js_sidebar_nav(
                &SidebarNavSpec::new(groups.clone()).with_value("projects"),
                theme,
            ),
        ))
        .child(group(
            "No selection",
            secondary,
            js_sidebar_nav(&SidebarNavSpec::new(groups), theme),
        ))
}
