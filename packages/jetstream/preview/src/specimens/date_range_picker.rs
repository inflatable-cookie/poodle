...
pub fn render(tree: &mut UiTree, theme: &dyn ThemeProvider) -> UiNodeId {
    ...
    section_label(tree, root, "With Label", text_secondary);
    {
        ...
        let lbl = tree.create_node(Widget::Label { text: "Date Range".to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()), NodeStyle {
...
fn date_field(tree: &mut UiTree, parent: UiNodeId, text: &str, bg: glam::Vec4, border: glam::Vec4, fg: glam::Vec4) {
    let input = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new().with_width(LayoutSizing::Fixed(140.0)).with_height(LayoutSizing::Fixed(32.0)).with_padding(LayoutEdges { top: 0.0, right: 10.0, bottom: 0.0, left: 10.0 }).with_alignment(MainAxisAlignment::Start, CrossAxisAlignment::Center)), NodeStyle {
        background: Some(bg),
