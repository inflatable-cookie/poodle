...
pub fn render(tree: &mut UiTree, theme: &dyn ThemeProvider) -> UiNodeId {
    ...
    label(tree, root, "In Context (above heading)", text_secondary);
    {
        ...
        let heading = tree.create(Widget::Label { text: "Welcome to the platform".to_string() }, UiStyle {
            text_color: Some(text_primary), text_size: Some(18.0), ..UiStyle::default()
        });
        tree.add_child(col, heading);

        let body = tree.create(Widget::Label { text: "Learn the basics and get up to speed quickly.".to_string() }, UiStyle {
