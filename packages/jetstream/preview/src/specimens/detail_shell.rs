...
pub fn render(tree: &mut UiTree, theme: &dyn ThemeProvider) -> UiNodeId {
    ...
    let root = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
        ...
        .with_gap(24.0)), NodeStyle::default());
    ...
    section_label(tree, root, "Standard Detail Shell", text_secondary);
    {
        ...
        let header = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
            ...
            .with_padding(LayoutEdges { top: 16.0, right: 16.0, bottom: 16.0, left: 16.0 })
        ...
        let header_left = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
            .with_direction(LayoutDirection::Column).with_gap(4.0)),
            NodeStyle::default());
        tree.add_child(header, header_left);

        let breadcrumb = tree.create_node(Widget::Label { text: "Documents > Project Brief".to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()),
            NodeStyle { text_color: Some(text_secondary), text_size: Some(10.0), ..NodeStyle::default() });
        tree.add_child(header_left, breadcrumb);

        let title = tree.create_node(Widget::Label { text: "Project Brief".to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()),
        ...
        let actions = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
            .with_direction(LayoutDirection::Row).with_gap(6.0)),
            NodeStyle::default());
        ...
        let more = tree.create_node(Widget::Button {
            label: "⋯".to_string(), pressed: false, hovered: false,
        }, pug_jetstream::map_layout(&LayoutIntent::new()
            .with_width(LayoutSizing::Fixed(28.0)).with_height(LayoutSizing::Fixed(28.0))
            .with_alignment(MainAxisAlignment::Center, CrossAxisAlignment::Center)),
            NodeStyle { corner_radii: [6.0; 4], border_color: Some(border), border_width: 1.0, text_color: Some(text_primary), text_size: Some(14.0), focusable: true, ..NodeStyle::default() });
        ...
        let content = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
            .with_direction(LayoutDirection::Column)
            .with_padding(LayoutEdges::uniform(16.0)).with_gap(16.0)),
            NodeStyle::default());
