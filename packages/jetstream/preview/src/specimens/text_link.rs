...
//!   - underline treatment + focus ring: JsEl exposes no text-decoration or
...
pub fn render(theme: &JetstreamThemeProvider) -> JsEl {
    ...
    div().flex_col().gap(24.0)
        // ── Inline prose ──
        .child(group("Inline prose", secondary,
            div().flex_row().flex_wrap().gap(4.0).items_center()
                ...
                .child(label("before wiring the production route.").text_color(primary).text_size(13.0))
        ))
        // ── Tones ──
        .child(group("Tones", secondary,
            div().flex_row().flex_wrap().gap(16.0).items_center()
                .child(js_text_link(
        ...
        .child(group("Button action", secondary,
