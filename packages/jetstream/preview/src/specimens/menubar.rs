...
fn full_items() -> Vec<MenubarEntry> {
    vec![
        ...
        MenubarEntry::new("window", "Window", vec![MenuEntry::new("minimize", "Minimize")])
            .with_disabled(true),
    ]
}
...
pub fn render(theme: &JetstreamThemeProvider) -> JsEl {
    ...
    div().flex_col().gap(24.0)
        ...
        .child(group("Application menu bar (File open)", secondary,
            js_menubar(
        ...
        .child(group("Edit open (disabled item)", secondary,
            js_menubar(
                &MenubarSpec::new(full_items())
                    .with_value("edit")
                    .with_aria_label("Application menu"),
        ...
        .child(group("View open (checkbox + radio) + disabled trigger", secondary,
            js_menubar(
                &MenubarSpec::new(full_items())
                    .with_value("view")
                    .with_aria_label("Application menu"),
        ...
        .child(group("Size ladder", secondary,
            div().flex_col().gap(16.0)
                .child(ladder(theme, "XS", MenubarSpec::new(ladder_items()).with_size(ControlSize::Xs)))
                .child(ladder(theme, "SM", MenubarSpec::new(ladder_items()).with_size(ControlSize::Sm)))
                .child(ladder(theme, "MD", MenubarSpec::new(ladder_items()).with_size(ControlSize::Md)))
                .child(ladder(theme, "LG", MenubarSpec::new(ladder_items()).with_size(ControlSize::Lg)))
                .child(ladder(theme, "XL", MenubarSpec::new(ladder_items()).with_size(ControlSize::Xl)))
        ))
        // Density ladder — trigger padding tightens/loosens, height unchanged.
        .child(group("Density ladder", secondary,
            div().flex_col().gap(16.0)
                .child(ladder(theme, "COMPACT", MenubarSpec::new(ladder_items()).with_density(ControlDensity::Compact)))
                .child(ladder(theme, "DEFAULT", MenubarSpec::new(ladder_items()).with_density(ControlDensity::Default)))
                .child(ladder(theme, "COMFORTABLE", MenubarSpec::new(ladder_items()).with_density(ControlDensity::Comfortable)))
        ))
}
