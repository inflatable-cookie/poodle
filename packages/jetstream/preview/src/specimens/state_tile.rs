...
pub fn render(theme: &JetstreamThemeProvider) -> El {
    ...
    div().flex_col().gap(24.0)
        .child(group("Default (no trend)", secondary,
            div().flex_row().gap(16.0)
                .child(js_state_tile(&StateTileSpec::new("Total Records", "8,402"), theme))
                .child(js_state_tile(&StateTileSpec::new("Open Issues", "37"), theme))
        ))
        .child(group("With trend (up / down)", secondary,
            div().flex_row().gap(16.0)
                .child(
                    js_state_tile(
        ...
        .child(group("Neutral trend (arbitrary string)", secondary,
            div().flex_row().gap(16.0)
                .child(
                    js_state_tile(
        ...
        .child(group("With sparkline (host renders chart in slot)", secondary,
            js_state_tile(
