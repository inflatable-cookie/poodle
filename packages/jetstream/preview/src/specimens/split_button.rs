...
pub fn render(tree: &mut UiTree, theme: &dyn ThemeProvider) -> UiNodeId {
    ...
    let sizes_col = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
        .with_direction(LayoutDirection::Column)
        .with_gap(8.0)),
        NodeStyle::default());
    ...
    for &(lbl, h, ts) in &[("Small", 24.0_f32, 11.0_f32), ("Medium", 32.0, 12.0), ("Large", 40.0, 14.0)] {
        ...
        let main = tree.create_node(Widget::Button {
            label: lbl.to_string(),
            pressed: false,
            hovered: false,
            ...
            NodeStyle { corner_radii: [0.0; 4], background: Some(accent), text_color: Some(text_inverse), text_size: Some(ts), focusable: true, ..NodeStyle::default() });
        ...
        let sep = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
            ...
            NodeStyle { background: Some(theme_bridge::tint(text_inverse, 0.3)), ..NodeStyle::default() });
        ...
        let trigger = tree.create_node(Widget::Button {
            ...
            .with_width(LayoutSizing::Fixed(h))
            .with_height(LayoutSizing::Fixed(h))
            .with_alignment(MainAxisAlignment::Center, CrossAxisAlignment::Center)),
            NodeStyle { corner_radii: [0.0; 4], background: Some(accent), text_color: Some(text_inverse), text_size: Some(ts), focusable: true, ..NodeStyle::default() });
...
fn split_button_group(
    ...
    let main = tree.create_node(Widget::Button {
        ...
        .with_alignment(MainAxisAlignment::Center, CrossAxisAlignment::Center)),
        NodeStyle { corner_radii: [0.0; 4], background: Some(bg), text_color: Some(text), text_size: Some(12.0), focusable: true, ..NodeStyle::default() });
    tree.add_child(group, main);
    ...
    let sep = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new()
        .with_width(LayoutSizing::Fixed(1.0))
        .with_height(LayoutSizing::Grow)),
        NodeStyle { background: Some(sep_color), ..NodeStyle::default() });
    tree.add_child(group, sep);
