...
pub fn render(theme: &JetstreamThemeProvider) -> El {
    ...
    div().flex_col().gap(24.0)
        ...
        .child(group("With step", secondary,
            row(js_slider(&{
        ...
        .child(group("Value (low / mid / high)", secondary,
        ...
        .child(group("Custom min / max + step (50–200, step 25)", secondary,
            row(js_slider(&{
        ...
        .child(group("Densities", secondary,
            div().flex_col().gap(12.0)
                .child(row(js_slider(&s(60.0).with_density(ControlDensity::Compact), theme)))
                .child(row(js_slider(&s(60.0).with_density(ControlDensity::Default), theme)))
                .child(row(js_slider(&s(60.0).with_density(ControlDensity::Comfortable), theme)))
        ))
}
