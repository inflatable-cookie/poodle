...
pub fn render(tree: &mut UiTree, theme: &dyn ThemeProvider) -> UiNodeId {
    ...
    {
        let row = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new().with_direction(LayoutDirection::Row).with_gap(4.0).with_alignment(MainAxisAlignment::Start, CrossAxisAlignment::Center)), NodeStyle::default());
        tree.add_child(root, row);

        let prefix = tree.create_node(Widget::Label { text: "Use the ".to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()), NodeStyle {
        ...
        let suffix = tree.create_node(Widget::Label { text: " function".to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()), NodeStyle {
            text_color: Some(text_primary), text_size: Some(12.0), ..NodeStyle::default()
        });
    ...
    {
        let row = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new().with_direction(LayoutDirection::Row).with_gap(6.0).with_alignment(MainAxisAlignment::Start, CrossAxisAlignment::Center)), NodeStyle::default());
        tree.add_child(root, row);
    ...
    {
        let block = tree.create_node(Widget::Panel, pug_jetstream::map_layout(&LayoutIntent::new().with_direction(LayoutDirection::Column).with_width(LayoutSizing::Fixed(320.0)).with_padding(LayoutEdges::uniform(12.0)).with_gap(2.0)), NodeStyle {
        ...
        for line in &[
            ...
            let l = tree.create_node(Widget::Label { text: line.to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()), NodeStyle {
    ...
    label(tree, root, "With Line Numbers", text_secondary);
    {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            let num = tree.create_node(Widget::Label { text: format!("{}", i + 1) }, pug_jetstream::map_layout(&LayoutIntent::new().with_width(LayoutSizing::Fixed(16.0))), NodeStyle {
                text_color: Some(text_secondary), text_size: Some(11.0),
                ..NodeStyle::default()
            });
            tree.add_child(row, num);

            let code = tree.create_node(Widget::Label { text: line.to_string() }, pug_jetstream::map_layout(&LayoutIntent::new()), NodeStyle {
                text_color: Some(text_primary), text_size: Some(11.0), ..NodeStyle::default()
            });
...
fn inline_code(tree: &mut UiTree, parent: UiNodeId, text: &str, bg: glam::Vec4, border: glam::Vec4, fg: glam::Vec4) {
    let code = tree.create_node(Widget::Label { text: text.to_string() }, pug_jetstream::map_layout(&LayoutIntent::new().with_padding(LayoutEdges { top: 1.0, right: 6.0, bottom: 1.0, left: 6.0 })), NodeStyle {
        corner_radii: [4.0; 4],
        background: Some(bg),
        border_color: Some(border),
        border_width: 1.0,
