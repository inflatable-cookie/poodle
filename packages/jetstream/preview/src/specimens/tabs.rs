...
pub fn render(state: &AppState, theme: &JetstreamThemeProvider) -> El {
    ...
    div()
        ...
        .child(group(
            "With count badges",
            secondary,
            js_tabs(&TabsSpec::new(items_counts).with_value("inbox"), theme),
        ))
        // ── Disabled tab ─────────────────────────────────────────────────
        .child(group(
        ...
        // ── Closable card tabs ───────────────────────────────────────────
        .child(group(
            "Card — closable (file tabs with close x)",
            secondary,
            js_tabs(
        ...
        .child(group(
            "Reorder — drag a tab to reorder (live)",
            secondary,
        ...
        .child(group(
            "Reorder — dragging 'details', over 'settings' (Card)",
            secondary,
            js_tabs(
                &TabsSpec::new(items.clone())
                    .with_variant(TabVariant::Card)
        ...
        // ── Full-width ───────────────────────────────────────────────────
        .child(group(
            "Full-width (tabs flex to fill the row)",
            secondary,
        ...
        .child(group(
            "Vertical orientation (Block, icon-only)",
            secondary,
            js_tabs(
        ...
        .child(group(
            ...
            div()
                ...
                .gap(12.0)
                .child(js_tabs(
                    &density_spec(&items_detail, ControlDensity::Compact),
                    theme,
                ))
                .child(js_tabs(
                    &density_spec(&items_detail, ControlDensity::Default),
                    theme,
                ))
                .child(js_tabs(
                    &density_spec(&items_detail, ControlDensity::Comfortable),
                    theme,
                )),
        ))
}
