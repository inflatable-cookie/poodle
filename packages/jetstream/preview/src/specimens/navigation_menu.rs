...
pub fn render(theme: &JetstreamThemeProvider) -> JsEl {
    ...
    div().flex_col().gap(24.0)
        ...
        .child(group("Horizontal navigation (active = Components)", secondary,
            js_navigation_menu(
        ...
        .child(group("Active = Tokens", secondary,
            js_navigation_menu(
                &NavigationMenuSpec::new(full_items())
                    .with_value("tokens")
                    .with_aria_label("Main navigation"),
        ...
        .child(group("Size ladder", secondary,
            div().flex_col().gap(16.0)
                .child(ladder(theme, "XS", NavigationMenuSpec::new(ladder_items()).with_size(ControlSize::Xs)))
                .child(ladder(theme, "SM", NavigationMenuSpec::new(ladder_items()).with_size(ControlSize::Sm)))
                .child(ladder(theme, "MD", NavigationMenuSpec::new(ladder_items()).with_size(ControlSize::Md)))
                .child(ladder(theme, "LG", NavigationMenuSpec::new(ladder_items()).with_size(ControlSize::Lg)))
                .child(ladder(theme, "XL", NavigationMenuSpec::new(ladder_items()).with_size(ControlSize::Xl)))
        ))
        // Density ladder — trigger padding tightens/loosens, height unchanged.
        .child(group("Density ladder", secondary,
            div().flex_col().gap(16.0)
                .child(ladder(theme, "COMPACT", NavigationMenuSpec::new(ladder_items()).with_density(ControlDensity::Compact)))
                .child(ladder(theme, "DEFAULT", NavigationMenuSpec::new(ladder_items()).with_density(ControlDensity::Default)))
                .child(ladder(theme, "COMFORTABLE", NavigationMenuSpec::new(ladder_items()).with_density(ControlDensity::Comfortable)))
        ))
}

fn ladder(theme: &JetstreamThemeProvider, lbl: &str, spec: NavigationMenuSpec) -> JsEl {
    let secondary = resolve_color(theme, "color.text.secondary");
