...
pub fn render(theme: &JetstreamThemeProvider) -> JsEl {
    ...
    div().flex_col().gap(rem_to_px(2.0))
        // ── Bottom placement ──────────────────────────────────────────
        .child(group("Bottom (surface below anchor)", secondary,
            div().flex_row().items_start().gap(rem_to_px(3.0))
                // BottomStart: panel aligns to anchor left edge
                .child(
                    div().flex_col().gap(rem_to_px(0.5)).items_start()
                        .child(label("BottomStart").text_color(secondary).text_size(rem_to_px(0.6875)))
                ...
                .child(
                    div().flex_col().gap(rem_to_px(0.5)).items_start()
                        .child(label("Bottom").text_color(secondary).text_size(rem_to_px(0.6875)))
                        .child(js_floating_overlay(
                            anchor("Trigger"),
                ...
                .child(
                    div().flex_col().gap(rem_to_px(0.5)).items_start()
                        .child(label("BottomEnd").text_color(secondary).text_size(rem_to_px(0.6875)))
                        .child(js_floating_overlay(
                            anchor("Trigger"),
        ...
        // ── Right placement ───────────────────────────────────────────
        .child(group("Right (surface right of anchor)", secondary,
            div().flex_row().items_start().gap(rem_to_px(3.0))
                .child(
                    div().flex_col().gap(rem_to_px(0.5)).items_start()
                        .child(label("RightStart").text_color(secondary).text_size(rem_to_px(0.6875)))
                        .child(js_floating_overlay(
                            anchor("Trigger"),
                ...
                .child(
                    div().flex_col().gap(rem_to_px(0.5)).items_start()
                        .child(label("RightEnd").text_color(secondary).text_size(rem_to_px(0.6875)))
                        .child(js_floating_overlay(
                            anchor("Trigger"),
        ...
        .child(group("No surface (anchor only — overlay hidden)", secondary,
            js_floating_overlay(
