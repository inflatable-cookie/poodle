...
//! rendered from the generated JSON, never restated here. Every button
//! renders through the node tier (`poodle_render::button` →
//! `poodle_gpui_node_backend::to_gpui`).

use crate::app_state::AppState;
use crate::conformance_support::{enum_values, spec_from_fixture, CASES, INTERFACE};
...
use poodle_specs::{ButtonSpec, EyebrowSpec};
use serde_json::Value;

/// A node-tier Button with no handler (the corpus projection is static).
...
fn render_corpus_groups(theme: &GpuiThemeProvider) -> Div {
    ...
    for case in &case_list {
        ...
        root = root.child(row(theme, &caption, spec_from_fixture(&fixture)));
...
fn row(theme: &GpuiThemeProvider, caption: &str, spec: ButtonSpec) -> Div {
    ...
    div()
        ...
        .child(node_button_static(spec, theme))
}
