...
pub(crate) fn render(state: &AppState, cx: &mut Context<PreviewRoot>) -> Div {
    ...
    let examples = div()
        ...
        .child(
            div()
                ...
                .child(Eyebrow::from_spec(
                    EyebrowSpec::new().with_content("Determinate"),
                    theme,
                ))
                .child(Progress::from_spec(
                    ProgressSpec::new().with_value(0.0),
                    theme,
                ))
                .child(Progress::from_spec(
                    ProgressSpec::new().with_value(35.0),
                    theme,
                ))
                .child(Progress::from_spec(
                    ProgressSpec::new().with_value(72.0),
                    theme,
                ))
                .child(Progress::from_spec(
                    ProgressSpec::new().with_value(100.0),
                    theme,
                )),
        )
        // --- Indeterminate ---
        ...
        .child(
            div()
                ...
                .gap(px(8.0))
                .child(Eyebrow::from_spec(
                    EyebrowSpec::new().with_content("Custom max"),
                    theme,
                ))
                .child(Progress::from_spec(
                    {
    ...
    specimen_layout(
        ...
        |_density, theme: &GpuiThemeProvider| {
            Progress::from_spec(ProgressSpec::new().with_value(60.0), theme).into_any_element()
        },
    )
}
