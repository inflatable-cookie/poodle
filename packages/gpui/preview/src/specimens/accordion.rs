...
use pug_primitives::{AccordionSpec, AccordionItemSpec, AccordionSelectionValue, EyebrowSpec};
use pug_gpui_components::{Accordion, Eyebrow};
...
pub(crate) fn render(state: &AppState, cx: &mut Context<PreviewRoot>) -> Div {
    ...
    let multi_content: Vec<(&str, &str)> = vec![
        ("design", "Components consume semantic tokens like --pug-color-text-primary and --pug-size-control-height rather than hard-coded values. Switching themes at runtime updates every component instantly without re-rendering."),
