...
fn main() {
    // Live `Application` construction is blocked on this spike: 1.19 removed
    // `Application::new()`, and `gpui-platform-gpui-unofficial` cannot build
    // from crates.io because `gpui-apple`'s build.rs looks for a sibling
    // `../gpui-unofficial` that cargo's registry layout does not provide.
    // Headless tests still compile through `TestAppContext`.
