...
use poodle_specs::{
    DataTableSpec, DetailSectionSpec, DetailShellSpec, FilterToolbarSpec, MediaPreviewSpec,
    MediaThumbnailSpec, PaginationSummarySpec, PickerShellSpec, RelationPickerSpec,
...
mod tests {
    ...
    fn picker_shell() {
        assert_eq!(
            a().render(&PickerShellSpec::new("Pick"), &s(), &t())
                .spec_type,
            "PickerShellSpec"
        );
    }
    ...
    fn media_thumbnail() {
        assert_eq!(
            a().render(&MediaThumbnailSpec::new(MediaKind::Image), &s(), &t())
                .spec_type,
            "MediaThumbnailSpec"
        );
    }
    ...
    fn media_preview() {
        assert_eq!(
            a().render(&MediaPreviewSpec::new(MediaKind::Audio, "Test"), &s(), &t())
                .spec_type,
            "MediaPreviewSpec"
        );
    }
