...
fn current_expanded_prefers_controlled_over_default() {
    ...
    let controlled = spec
        .clone()
        .with_expanded_values(vec!["src/components".into()]);
    assert_eq!(
        controlled.current_expanded(),
...
fn selection_membership() {
    let spec = sample().with_selected_values(vec!["src/index.ts".into(), "README.md".into()]);
...
fn visible_rows_and_nav_helpers() {
    ...
    assert_eq!(
        spec.prev_visible("src/index.ts").as_deref(),
        Some("src/components")
    );
...
fn check_state_cascades_from_descendants() {
    ...
    let nodes = vec![
        TreeNode::branch(
            "dir",
            "dir",
            vec![TreeNode::new("dir/a", "a"), TreeNode::new("dir/b", "b")],
        ),
        TreeNode::new("c", "c"),
    ];
    let dir = &nodes[0];
    ...
    let full =
        TreeSpec::new(nodes.clone()).with_checked_values(vec!["dir/a".into(), "dir/b".into()]);
...
fn reorder_tree() -> Vec<TreeNode> {
    vec![
        ...
        TreeNode::branch(
            "b",
            "b",
            vec![TreeNode::new("b1", "b1"), TreeNode::new("b2", "b2")],
        ),
        TreeNode::new("c", "c"),
    ]
}
...
fn reorder_state_and_siblings() {
    ...
    let s = spec.with_reorderable(true).with_drag(
        Some("a".into()),
        Some("c".into()),
        DropPosition::After,
    );
