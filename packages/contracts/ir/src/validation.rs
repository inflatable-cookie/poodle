...
use crate::{
    Axes, Identifier, IrModel, PartKind, PermittedSubset, Prop, PropType, SceneAxisKind, Value,
};
...
fn validate_component_parts(
    ...
    for part in &component.parts {
        ...
        match &part.kind {
            ...
            PartKind::ConditionalDocumented { condition, .. } => {
                if condition.trim().is_empty() {
                    findings.push(Finding::new(
                        FindingKind::ImpossibleBinding,
                        scope("part", &part.id),
                        format!(
                            "documented-conditional part '{}' carries an empty condition; the \
                             documented condition must name when the part renders (g13.017 R1 \
                             bucket 2)",
                            part.id
                        ),
                    ));
...
fn validate_component_attributes(
    ...
    for attribute in &component.attributes {
        ...
        // A valued attribute without a source is runtime-derived (e.g.
        // `data-state` active/idle); its value domain is the description's
        // prose (g13.017 R1 bucket 2 — the expression form is gone).
    }
}
