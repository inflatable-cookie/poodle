...
use crate::{
    AttributeForm, Axes, Expr, ExprLiteral, ExprOperand, Identifier, IrModel, PartKind,
    PermittedSubset, Prop, PropType, SceneAxisKind, Value, VisualFieldKind,
};
...
pub enum FindingKind {
    ...
    /// A type error in a bounded expression — an operator applied to
    /// operands of the wrong type, or an expression whose result does not
    /// match its slot's expected type (spec 063 "The bounded expression
    /// vocabulary (normative)"; card 012).
    #[serde(rename = "expression-type-error")]
    ExpressionTypeError,
    /// An expression reference to an identifier outside the slot's declared
    /// scope — not a prop, controlled state, VisualState field, part, or
    /// axis of the enclosing component, or a member literal naming a member
    /// the shared type does not define (spec 063 "Operands"; card 012).
    #[serde(rename = "unresolved-expression-reference")]
...
fn validate_conformance_vectors(model: &IrModel, findings: &mut Vec<Finding>) {
    ...
    validate_vector_guards(model, findings);
}
...
fn validate_component(
    ...
    validate_component_expressions(model, component, findings, &scope);
}
...
fn validate_component_props(
    ...
    for prop in &component.props {
        ...
        // A prop defaults either by value or by expression, never both
        // (spec 063 "prop default and axis fallback resolution"; CROSS-02).
        if prop.default.is_some() && prop.default_expr.is_some() {
            findings.push(Finding::new(
                FindingKind::ImpossibleBinding,
                scope("prop", &prop.id),
                format!(
                    "prop '{}' declares both a default value and a default expression; pick one \
                     way to default the prop (CROSS-02)",
                    prop.id
                ),
            ));
        }
...
fn validate_component_parts(
    ...
    for part in &component.parts {
        ...
        match &part.kind {
            ...
            PartKind::ConditionalExpr { .. } => {
                // The expression is type-checked as a boolean render
                // condition in validate_component_expressions.
            }
...
fn validate_component_attributes(
    ...
    for attribute in &component.attributes {
        ...
        if attribute.form == AttributeForm::Valued
            && attribute.source.is_none()
            && attribute.value.is_none()
        {
            findings.push(Finding::new(
                ...
                scope("attribute", &attribute.id),
                format!(
                    "valued attribute '{}' has neither a source nor a value expression; a valued \
                     attribute needs a derivation to emit (CROSS-13)",
                    attribute.id
                ),
            ));
        }
        // A source and a value expression are two derivations of the same
        // value; pick one (CROSS-13; spec 063 expression slots).
        if attribute.source.is_some() && attribute.value.is_some() {
            findings.push(Finding::new(
                FindingKind::ImpossibleBinding,
                scope("attribute", &attribute.id),
                format!(
                    "attribute '{}' declares both a source and a value expression; pick one \
                     derivation for the emitted value (CROSS-13)",
                    attribute.id
                ),
            ));
        }
        // Presence-only attributes emit no value, so a value expression is
        // meaningless on them (CROSS-13 "presence-only vs valued").
        if attribute.form == AttributeForm::PresenceOnly && attribute.value.is_some() {
            findings.push(Finding::new(
                FindingKind::ImpossibleBinding,
                scope("attribute", &attribute.id),
                format!(
                    "presence-only attribute '{}' declares a value expression; presence-only \
                     attributes emit no value — use a condition or a valued form (CROSS-13)",
                    attribute.id
                ),
            ));
        }
...
// ---------------------------------------------------------------------------
// Bounded expression type checking (spec 063 "The bounded expression
// vocabulary (normative)"; card 012)
// ---------------------------------------------------------------------------

/// Static type of an expression, used to type-check the bounded vocabulary
/// against declared prop, state, and projection types (spec 063; card 012
/// "Typed"). `Unknown` marks a subexpression whose reference already failed
/// to resolve; its finding was reported, so no further checks cascade.
#[derive(Debug, Clone, PartialEq, Eq)]
enum ExprType {
    /// A boolean value.
    Bool,
    /// A numeric value — ordering-safe integers (prop `Number`, VisualState
    /// `Number` fields, `Int` literals).
    Number,
    /// A string value — `is_empty` target.
    String,
    /// A member of the named shared enumerated type (g13-b003 R6.1).
    Member(Identifier),
    /// A list value — `is_empty` target.
    List(Box<ExprType>),
    /// A value with no expression operations in this vocabulary (pairs and
    /// opaque payloads; spec 063 excludes indexing/field access).
    Opaque,
    /// An unresolved reference; the finding is reported separately.
    Unknown,
}

/// The declared names an expression in a scope may reference (spec 063
/// "Operands": "a declared prop, a declared state field, a VisualState
/// projection field, a slot's presence, a resolved axis value"). Component
/// slots build this from the component's declarations; conformance-vector
/// guards use the model-only scope, which names no component state (vectors
/// are shared machine semantics).
struct ExprEnv<'a> {
    /// The model, for shared-type resolution of member literals.
    model: &'a IrModel,
    /// Declared prop ids → the prop (`CROSS-02`).
    props: BTreeMap<&'a str, &'a Prop>,
    /// Declared controlled-state ids → the state's type, inferred from the
    /// controlled prop (`CROSS-04`).
    states: BTreeMap<&'a str, ExprType>,
    /// VisualState projection field ids → the field's kind (`CROSS-14`).
    visual: BTreeMap<&'a str, ExprType>,
    /// Part ids — slot-presence operands (`CROSS-12`).
    parts: BTreeSet<&'a str>,
    /// The component's axes, for resolved-axis operands; `None` in scopes
    /// that declare no component state.
    axes: Option<&'a Axes>,
}

impl<'a> ExprEnv<'a> {
    /// A scope with no component state — conformance-vector guards (shared
    /// machine semantics name no component state; guards over machine state
    /// are vector machines, `CROSS-19`, `RNG-02`, `TXT-09`).
    fn model_scope(model: &'a IrModel) -> Self {
        Self {
            model,
            props: BTreeMap::new(),
            states: BTreeMap::new(),
    ...
    /// Whether the named axis is declared on the enclosing component.
    fn axis_declared(&self, axis: &str) -> bool {
        match self.axes {
            Some(axes) => match axis {
                "size" => axes.size.is_some(),
                "density" => axes.density.is_some(),
                "orientation" => axes.orientation.is_some(),
                _ => false,
            },
            None => false,
        }
...
/// Builds the expression scope for a component's slots (spec 063 operands).
fn expression_env<'a>(
    model: &'a IrModel,
    component: &'a crate::ComponentDefinition,
) -> ExprEnv<'a> {
    let props: BTreeMap<&str, &Prop> = component.props.iter().map(|p| (p.id.as_str(), p)).collect();
    let states = component
        .controlled_state
        .iter()
        .filter_map(|state| {
            let controlled = component.props.iter().find(|p| p.id == state.controlled)?;
            Some((
                state.id.as_str(),
                prop_type_expr_type(&controlled.prop_type),
            ))
        })
        .collect();
    let visual = component
        .visual_state
        .iter()
        .flat_map(|state| state.fields.iter())
        .map(|field| (field.id.as_str(), visual_kind_expr_type(&field.kind)))
        .collect();
    let parts = component.parts.iter().map(|p| p.id.as_str()).collect();
    ExprEnv {
        model,
        props,
        states,
        visual,
        parts,
        axes: Some(&component.axes),
    }
}

/// The expression type a declared prop carries (`CROSS-02`).
fn prop_type_expr_type(prop_type: &PropType) -> ExprType {
    match prop_type {
        PropType::String => ExprType::String,
        PropType::Number => ExprType::Number,
        PropType::Bool => ExprType::Bool,
        PropType::Shared(id) => ExprType::Member(id.clone()),
        // Pairs and opaque payloads carry no expression operations in this
        // vocabulary (spec 063 excludes indexing/field access).
        PropType::Pair(_) | PropType::Opaque => ExprType::Opaque,
        PropType::List(inner) => ExprType::List(Box::new(prop_type_expr_type(inner))),
    }
}

/// The expression type a VisualState projection field carries (`CROSS-14`).
fn visual_kind_expr_type(kind: &VisualFieldKind) -> ExprType {
    match kind {
        VisualFieldKind::Number => ExprType::Number,
        VisualFieldKind::Bool => ExprType::Bool,
        VisualFieldKind::String => ExprType::String,
        VisualFieldKind::Enum(id) => ExprType::Member(id.clone()),
        VisualFieldKind::Pair => ExprType::Opaque,
    }
}

/// The shared type a resolved axis value names (spec 063 operand "a
/// resolved axis value"). The pilot components type their size, density,
/// and orientation props against these first-class shared types (`CROSS-07`
/// xs–xl ladder, `CROSS-08` densities, `CROSS-11` orientation), so a
/// resolved axis value is a member of that type.
fn resolved_axis_shared_type(axis: &str) -> Option<&'static str> {
    match axis {
        "size" => Some("control-size"),
        "density" => Some("control-density"),
        "orientation" => Some("orientation"),
        _ => None,
    }
}

/// Whether the expression is a bare literal operand.
fn is_literal_expr(expr: &Expr) -> bool {
    matches!(expr, Expr::Operand(ExprOperand::Literal(_)))
}

/// Reports an unresolved expression reference with an actionable message
/// naming the missing identifier and the operand vocabulary (spec 063).
fn unresolved_reference(where_id: &str, id: &str, findings: &mut Vec<Finding>) {
    findings.push(Finding::new(
        FindingKind::UnresolvedExpressionReference,
        where_id.to_owned(),
        format!(
            "expression references '{id}' which is not declared in this scope; expressions may \
             reference declared props, controlled states, VisualState fields, parts (slot \
             presence), and resolved axis values (spec 063 operands)"
        ),
    ));
}

/// Reports a member literal that does not resolve against its named shared
/// type (g13-b003 R6.1).
fn unresolved_member(where_id: &str, member: &str, shared_type: &str, findings: &mut Vec<Finding>) {
    findings.push(Finding::new(
        FindingKind::UnresolvedExpressionReference,
        where_id.to_owned(),
        format!(
            "expression member '{member}' is not a member of shared type '{shared_type}'; add it \
             to the shared type or fix the reference (g13-b003 R6.1)"
        ),
    ));
}

/// Pushes an expression type error and returns `Unknown` so the parent
/// stops checking this branch (the finding names the offending slot).
fn type_error(where_id: &str, findings: &mut Vec<Finding>, msg: String) -> ExprType {
    findings.push(Finding::new(
        FindingKind::ExpressionTypeError,
        where_id.to_owned(),
        msg,
    ));
    ExprType::Unknown
...
/// Type-checks an expression in a slot with an expected result type
/// (spec 063 slots: emission conditions, render conditions, defaults,
/// fallbacks, guards). `expected_desc` names the required shape for the
/// actionable message, e.g. "a boolean expression".
fn check_slot_expr(
    env: &ExprEnv<'_>,
    expr: &Expr,
    expected: ExprType,
    ...
    let actual = check_expr(env, expr, where_id, findings);
    if actual != ExprType::Unknown && actual != expected {
        type_error(
            where_id,
            findings,
            format!(
                "the expression has type {actual:?}, but the slot requires {expected_desc}; fix \
                 the expression to produce the slot's value type (spec 063)"
            ),
        );
    }
}

/// Type-checks an expression, returning its static type. Every type error
/// and unresolved reference is reported as a [`Finding`] at the authored
/// source with the offending slot identifier.
fn check_expr(
    env: &ExprEnv<'_>,
    expr: &Expr,
    where_id: &str,
    ...
    match expr {
        Expr::Operand(operand) => check_operand(env, operand, where_id, findings),
        Expr::Not(inner) => match check_expr(env, inner, where_id, findings) {
            ExprType::Bool => ExprType::Bool,
            ExprType::Unknown => ExprType::Unknown,
            other => type_error(
                where_id,
                findings,
                format!("'not' requires a boolean operand, got {other:?} (spec 063)"),
            ),
        },
        Expr::And(left, right) => check_logical(env, "and", left, right, where_id, findings),
        Expr::Or(left, right) => check_logical(env, "or", left, right, where_id, findings),
        Expr::Eq(left, right) => check_equality(env, "eq", left, right, where_id, findings),
        Expr::Ne(left, right) => check_equality(env, "ne", left, right, where_id, findings),
        Expr::IsNull(inner) | Expr::IsPresent(inner) => {
            check_nullability(env, inner, where_id, findings)
        }
        Expr::Coalesce(left, right) => {
            let left_t = check_expr(env, left, where_id, findings);
            let right_t = check_expr(env, right, where_id, findings);
            match (left_t, right_t) {
                ...
                (l, r) => type_error(
                    ...
                    findings,
                    format!(
                        "'coalesce' operands must carry the same type, got {l:?} and {r:?} \
                         (spec 063)"
                    ),
                ),
            }
        }
        Expr::Gt(left, right) => check_ordering(env, "gt", left, right, where_id, findings),
        Expr::Gte(left, right) => check_ordering(env, "gte", left, right, where_id, findings),
        Expr::Lt(left, right) => check_ordering(env, "lt", left, right, where_id, findings),
        Expr::Lte(left, right) => check_ordering(env, "lte", left, right, where_id, findings),
        Expr::IsEmpty(inner) => match check_expr(env, inner, where_id, findings) {
            ExprType::String | ExprType::List(_) => ExprType::Bool,
            ExprType::Unknown => ExprType::Unknown,
            other => type_error(
                where_id,
                findings,
                format!("'is_empty' applies to strings and collections, got {other:?} (spec 063)"),
            ),
        },
        Expr::If {
            condition,
            ...
            match check_expr(env, condition, where_id, findings) {
                ExprType::Bool => {}
                ExprType::Unknown => return ExprType::Unknown,
                other => {
                    return type_error(
                        where_id,
                        findings,
                        format!(
                            "'if' condition must be a boolean expression, got {other:?} (spec 063)"
                        ),
                    );
                }
            }
            let then_t = check_expr(env, then, where_id, findings);
            let else_t = check_expr(env, otherwise, where_id, findings);
            match (then_t, else_t) {
                (ExprType::Unknown, _) | (_, ExprType::Unknown) => ExprType::Unknown,
                (t, e) if t == e => t,
                (t, e) => type_error(
                    where_id,
                    findings,
                    format!("'if' arms must carry the same type, got {t:?} and {e:?} (spec 063)"),
                ),
            }
        }
    }
...
/// Type-checks a boolean conjunction/disjunction.
fn check_logical(
    env: &ExprEnv<'_>,
    op: &str,
    left: &Expr,
    ...
    match (left_t, right_t) {
        ...
        (l, r) => type_error(
            where_id,
            findings,
            format!("'{op}' requires boolean operands, got {l:?} and {r:?} (spec 063)"),
        ),
    }
}

/// Type-checks an ordering comparison — integers only (spec 063).
fn check_ordering(
    env: &ExprEnv<'_>,
    op: &str,
    left: &Expr,
    ...
    let right_t = check_expr(env, right, where_id, findings);
    match (left_t, right_t) {
        (ExprType::Number, ExprType::Number) => ExprType::Bool,
        (ExprType::Unknown, _) | (_, ExprType::Unknown) => ExprType::Unknown,
        (l, r) => type_error(
            where_id,
            findings,
            format!("'{op}' compares integers only, got {l:?} and {r:?} (spec 063)"),
        ),
    }
}

/// Type-checks equality/inequality — "against a literal or shared-type
/// member" (spec 063): the right side must be a literal or member, the left
/// side must be a state reference or compound expression, and both sides
...
fn check_equality(
    ...
    if !is_literal_expr(right) {
        return type_error(
            ...
            findings,
            format!(
                "'{op}' compares against a literal or shared-type member, but the right side is \
                 an expression; move the literal or member to the right (spec 063)"
            ),
        );
    }
    if is_literal_expr(left) {
        return type_error(
            ...
            findings,
            format!(
                "'{op}' left side must be a state reference or expression, not a literal \
                 (spec 063)"
            ),
        );
    }
    ...
    match (left_t, right_t) {
        (ExprType::Unknown, _) | (_, ExprType::Unknown) => ExprType::Unknown,
        (l, r) if l == r => ExprType::Bool,
        (l, r) => type_error(
            where_id,
            findings,
            format!(
                "'{op}' cannot compare {l:?} with {r:?}; both sides must carry the same type \
                 (spec 063)"
            ),
        ),
    }
...
/// Type-checks a nullability operator — it applies to a state reference,
/// never a literal or a computed expression (a computed value is never
/// null, so the operator would be constant and meaningless).
fn check_nullability(
    ...
    match inner {
        Expr::Operand(ExprOperand::Literal(_)) => type_error(
            where_id,
            findings,
            "'is_null'/'is_present' apply to a state reference, not a literal; literals are \
             never null (spec 063)"
                .to_owned(),
        ),
        Expr::Operand(operand) => match check_operand(env, operand, where_id, findings) {
            ExprType::Unknown => ExprType::Unknown,
            _ => ExprType::Bool,
        },
        _ => type_error(
            where_id,
            findings,
            "'is_null'/'is_present' apply to a state reference; a computed expression is never \
             null (spec 063)"
                .to_owned(),
        ),
    }
...
/// Type-checks an operand, resolving references against the scope and
/// member literals against the model's shared types (spec 063 "Operands").
fn check_operand(
    env: &ExprEnv<'_>,
    operand: &ExprOperand,
    where_id: &str,
    ...
    match operand {
        ExprOperand::Literal(literal) => match literal {
            ExprLiteral::Bool(_) => ExprType::Bool,
            ExprLiteral::Int(_) => ExprType::Number,
            ExprLiteral::String(_) => ExprType::String,
            ExprLiteral::Member {
                shared_type,
                member,
            } => {
                let shared = env.model.shared_type(shared_type.as_str());
                match shared {
                    None => {
                        findings.push(Finding::new(
                            FindingKind::UnresolvedExpressionReference,
                            where_id.to_owned(),
                            format!(
                                "expression member '{member}' references shared type \
                                 '{shared_type}' which is not defined; define it in shared_types \
                                 (g13-b003 R6.1)"
                            ),
                        ));
                        ExprType::Unknown
                    ...
                    Some(shared) => {
                        let ids = shared_member_ids(shared);
                        if ids.contains(&member.to_string()) {
                            ExprType::Member(shared_type.clone())
                        } else {
                            unresolved_member(
                                where_id,
                                member.as_str(),
        ...
        ExprOperand::Prop(id) => match env.props.get(id.as_str()) {
            Some(prop) => prop_type_expr_type(&prop.prop_type),
            None => {
                unresolved_reference(where_id, id.as_str(), findings);
                ExprType::Unknown
            }
        },
        ExprOperand::State(id) => match env.states.get(id.as_str()) {
            Some(expr_type) => expr_type.clone(),
            None => {
                unresolved_reference(where_id, id.as_str(), findings);
                ExprType::Unknown
            }
        },
        ExprOperand::Visual(id) => match env.visual.get(id.as_str()) {
            Some(expr_type) => expr_type.clone(),
            None => {
                unresolved_reference(where_id, id.as_str(), findings);
                ExprType::Unknown
        ...
        ExprOperand::Slot(id) => {
            if env.parts.contains(id.as_str()) {
                ExprType::Bool
            } else {
                unresolved_reference(where_id, id.as_str(), findings);
                ExprType::Unknown
        ...
        ExprOperand::ResolvedAxis(id) => match resolved_axis_shared_type(id.as_str()) {
            Some(shared) if env.axis_declared(id.as_str()) => {
                if env.model.shared_type(shared).is_none() {
                    findings.push(Finding::new(
                        FindingKind::InvalidReference,
                        where_id.to_owned(),
                        format!(
                            "the '{id}' axis resolves to shared type '{shared}' which is not \
                             defined; define it so the resolved axis value is typed (CROSS-07)"
                        ),
                    ));
                }
                ExprType::Member(Identifier::new(shared))
            }
            _ => {
                unresolved_reference(where_id, id.as_str(), findings);
                ExprType::Unknown
...
/// Type-checks every expression in the four component-scoped sanctioned
/// slots (spec 063 "Where expressions may appear"): attribute emission
/// conditions and values, part render conditions, prop defaults, and the
/// size axis fallback.
fn validate_component_expressions(
    model: &IrModel,
    component: &crate::ComponentDefinition,
    findings: &mut Vec<Finding>,
    scope: &dyn Fn(&str, &Identifier) -> String,
) {
    let env = expression_env(model, component);

    // State-derived attribute emission conditions and values (CROSS-13;
    // spec 063 expression slots).
    for attribute in &component.attributes {
        if let Some(condition) = &attribute.condition {
            check_slot_expr(
                ...
                condition,
                ExprType::Bool,
                "a boolean emission condition",
                &scope("attribute", &attribute.id),
                findings,
            );
        }
        if let Some(value) = &attribute.value {
            let _ = check_expr(&env, value, &scope("attribute", &attribute.id), findings);
        }
    }

    // Part render conditions (CROSS-12; spec 063 expression slots).
    for part in &component.parts {
        if let PartKind::ConditionalExpr { when, .. } = &part.kind {
            check_slot_expr(
                &env,
                when,
                ExprType::Bool,
                "a boolean part render condition",
                &scope("part", &part.id),
                findings,
            );
        }
    ...
    // Prop default expressions (CROSS-02; spec 063 expression slots).
    for prop in &component.props {
        if let Some(default_expr) = &prop.default_expr {
            let expected = prop_type_expr_type(&prop.prop_type);
            check_slot_expr(
                &env,
                default_expr,
                expected,
                &format!("a value matching the declared type of prop '{}'", prop.id),
                &scope("prop", &prop.id),
                findings,
            );
        }
    ...
    // Size axis fallback resolution (CROSS-07; spec 063 expression slots):
    // `size ?? resolveSemanticControlSize(sizeRole)` resolves to a
    // control-size member.
    if let Some(fallback) = component
        .axes
        .size
        .as_ref()
        .and_then(|axis| axis.fallback.as_ref())
    {
        check_slot_expr(
            &env,
            fallback,
            ExprType::Member(Identifier::new("control-size")),
            "a control-size member (the resolved axis value's domain)",
            &scope("size-axis", &Identifier::new("size")),
            findings,
        );
    }
}

/// Type-checks guard conditions on conformance-vector steps (spec 063
/// "guard conditions on transitions and effect-intents"). Vectors are
/// shared machine semantics that name no component state, so reference
/// operands resolve to findings here; guards over machine state are vector
/// machines (`CROSS-19`, `RNG-02`, `TXT-09`), not expressions.
fn validate_vector_guards(model: &IrModel, findings: &mut Vec<Finding>) {
    let env = ExprEnv::model_scope(model);
    for vector in &model.conformance_vectors {
        for step in &vector.steps {
