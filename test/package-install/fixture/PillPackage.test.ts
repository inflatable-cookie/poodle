...
import type { PillAppearance } from "@inflatable-cookie/poodle-svelte";

import PillHarness from "./PillHarness.svelte";

describe("packed @inflatable-cookie/poodle-svelte Pill", () => {
  it("exposes the corrected appearance axis from the packed root", () => {
    ...
    for (const appearance of appearances) {
      const view = render(PillHarness, { props: { appearance, tone: "success" } });
  ...
  it("defaults to the tint appearance with no fill axis", () => {
    const view = render(PillHarness, { props: {} });
    const root = view.container.querySelector(".poodle-pill") as HTMLElement;
